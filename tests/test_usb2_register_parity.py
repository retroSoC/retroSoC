"""Check the handwritten USB2 ABI against the RTL register contract."""
...
RTL_DEFINE = ROOT / "rtl/ip/usb/usb2_define.svh"
C_HEADER = ROOT / "crt/include/retrosoc/hal/usb2_regs.h"


def _rtl_values() -> dict[str, int]:
    values: dict[str, int] = {}
    pattern = re.compile(
        r"^`define\s+(APB4_USB2__\w+)\s+"
...
def _c_values() -> dict[str, int]:
    ...
    for line in C_HEADER.read_text(encoding="utf-8").splitlines():
        ...
        values[name.removeprefix("RS_USB2_ABI_")] = (
            int(hexadecimal, 16) if hexadecimal is not None else int(decimal, 10)
        )
    return values


def test_usb2_register_constants_match_expected_abi() -> None:
    rtl = _rtl_values()
    c_header = _c_values()
    assert c_header == {
        name.removeprefix("APB4_USB2__"): value for name, value in rtl.items()
    }
