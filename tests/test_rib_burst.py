"""Directed tests for the SoC-owned RIB burst building blocks."""
...
def test_bus_preserves_incr4_and_rejects_illegal_bursts(tmp_path: Path) -> None:
    ...
    source_list = tmp_path / "rib_burst_bus.fl"
    source_list.write_text(
        "\n".join(
            [
                ...
                str(ROOT / "rtl/mini/top/soc_rib_legacy_to_burst.sv"),
                str(ROOT / "rtl/mini/top/soc_rib_burst_to_legacy.sv"),
                str(ROOT / "rtl/mini/top/soc_rib_burst_error_slave.sv"),
                str(ROOT / "rtl/mini/top/soc_rib_burst_ram.sv"),
                str(ROOT / "rtl/mini/top/bus.sv"),
                str(ROOT / "tests/rtl/rib_burst_bus_tb.sv"),
                "",
            ]
        ),
        encoding="utf-8",
    )
    converted = tmp_path / "rib_burst_bus_tb.v"
    subprocess.run(
        [
            sys.executable,
    ...
    simulation = tmp_path / "rib_burst_bus_tb"
    subprocess.run(
        [iverilog, "-g2012", "-s", "rib_burst_bus_tb", "-o", str(simulation), str(converted)],
        check=True,
    )
...
def test_burst_sram_preserves_data_and_backpressure(tmp_path: Path) -> None:
    ...
    source_list = tmp_path / "soc_rib_burst_ram.fl"
    source_list.write_text(
        "\n".join(
            [
                "+define+SV_ASSRT_DISABLE",
                ...
                str(ROOT / "rtl/managed/clusterip/common/rtl/utils/spill_register.sv"),
                str(ROOT / "rtl/mini/top/soc_rib_burst_ram.sv"),
                str(ROOT / "tests/rtl/soc_rib_burst_ram_tb.sv"),
                "",
            ]
        ),
        encoding="utf-8",
    )
    converted = tmp_path / "soc_rib_burst_ram_tb.v"
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "rtl/mini/script/convt_sv2v.py"),
    ...
    simulation = tmp_path / "soc_rib_burst_ram_tb"
    subprocess.run(
        [iverilog, "-g2012", "-s", "soc_rib_burst_ram_tb", "-o", str(simulation), str(converted)],
        check=True,
    )
    result = subprocess.run([vvp, str(simulation)], text=True, capture_output=True, check=True)
    assert "RIB burst SRAM pipeline test passed" in result.stdout
