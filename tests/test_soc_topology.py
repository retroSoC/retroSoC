...
def test_topology_generates_complete_native_apb_and_gpio_bindings(tmp_path: Path) -> None:
    ...
    assert "assign s_slv_sel_d[17] = u_i2c1_nmi_if.valid;" in routes
    assert "assign s_xpi_region_sel[1] = (nmi.addr <= `SOC_ADDR_FLASH_END);" in routes
    assert "assign u_sdio_nmi_if.valid = 1'b0;" in routes
...
def test_generated_nmi_routes_select_and_return_the_expected_target(tmp_path: Path) -> None:
    ...
    source_list = tmp_path / "soc_topology_nmi.fl"
    source_list.write_text(
        "\n".join(
            [
                ...
                str(ROOT / "tests/rtl/soc_topology_nmi_tb.sv"),
                "",
            ]
        ),
        encoding="utf-8",
    )
    converted = tmp_path / "soc_topology_nmi_tb.v"
    subprocess.run(
        [
            sys.executable,
    ...
    simulation = tmp_path / "soc_topology_nmi_tb"
    subprocess.run(
        [
            iverilog,
            "-g2012",
    ...
    result = subprocess.run([vvp, str(simulation)], text=True, capture_output=True, check=True)
    assert "SoC topology NMI routing test passed" in result.stdout
