...
def test_topology_generates_complete_rib_apb_and_gpio_bindings(tmp_path: Path) -> None:
    ...
    assert interfaces.count("apb4_if u_") == 27
    ...
    assert apb_interfaces.count("apb4_if u_") == 11
    ...
    assert "`define SOC_IRQ_APB4_PERIPH_WIDTH 25" in irq_config
    ...
    assert "assign irq_o[14] = resource_irq_lp_i[0];" in rib_irq
    assert "assign irq_o[15] = opipsram.irq_o;" in rib_irq
    assert "assign irq_o[16] = resource_irq_lp_i[2];" in rib_irq
    ...
    assert "assign irq_o[7] = s_resource_fault_irq;" in apb_irq
...
def test_topology_preserves_default_irq_compatibility_mapping() -> None:
    ...
    assert mappings == [
        ...
        ("sdio0", "apb4_periph", 16, 10, "resource_irq_lp_i[2]"),
        ...
        ("dma", "apb4_periph", 14, 20, "resource_irq_lp_i[0]"),
        ("opipsram", "apb4_periph", 15, 22, "opipsram.irq_o"),
        ("sdio1", "apb4_periph", 17, 21, "resource_irq_lp_i[3]"),
        ("crypto", "apb4_periph", 18, 23, "s_crypto_irq"),
        ("usb2", "apb4_periph", 19, 24, "resource_irq_lp_i[1]"),
...
def test_ga2d_p3_topology_activates_the_shell_without_a_data_source() -> None:
    ...
    assert document["apb4_periph_targets"][-1]["name"] == "ga2d"
    assert document["apb4_periph_targets"][-1]["slot"] == 28
    assert document["apb4_periph_targets"][-1]["region"] == "APB4_GA2D"
    assert document["interrupts"][-1]["name"] == "ga2d"
    assert document["interrupts"][-1]["group_bit"] == 24
    assert document["interrupts"][-1]["core_bit"] == 32
...
def test_apu_p1_fixed_resource_and_hp_irq_allocations() -> None:
    ...
    resource_header = (ROOT / "crt/include/retrosoc/hal/resource.h").read_text(
        encoding="utf-8"
    )
