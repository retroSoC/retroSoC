...
// APU-P7 KWS frontend differential fixture. Each window is a plusarg-selected
// 32000-byte padded PCM image driven through operation 1 (one KWS memory
// window) on the real frontend/inference RTL path with the frozen APUM model
// resident in the KWS SRAM model reservation, exactly like the accuracy
// fixture. After each completed window the fixture prints the 490 computed
// MFCC scratch bytes (MFCC_HEX) and the window normalization peak (PEAK) for
// the host-side bit-accurate model comparison. Classifier history and
// counters are reset at every idle boundary between windows. The fixture
// fails closed on any DMA protocol, error, or timeout condition.
module apu_p7_frontend_dump_tb;
  ...
  task automatic run_window(input int unsigned window_index, input string pcm_dir,
    ...
    begin
      ...
      begin
        logic [7:0] dump_byte;
        $write("MFCC_HEX %0d ", window_index);
        for (int feature = 0; feature < 490; feature++) begin
          dump_byte = u_kws_sram_client.mem[
              8192+((`RETROSOC_APU_KWS__SCRATCH_MFCC_BASE+feature)/4)
          ][8*((`RETROSOC_APU_KWS__SCRATCH_MFCC_BASE+feature)%4)+:8];
          $write("%02x", dump_byte);
        end
        $display("");
        $display("PEAK %0d %0d", window_index, dut.s_window_peak_q);
      end
    end
  endtask
  ...
  initial begin
    ...
    $display("APU_P7_FRONTEND_DUMP_COMPLETE windows=%0d first=%0d", window_count, first_index);
    $finish;
  end
endmodule
