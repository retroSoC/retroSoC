...
def build_defines(args: argparse.Namespace) -> list[str]:
    defines = [
        ...
        f"+define+CORE_{args.core}",
        f"+define+IP_{args.ip}",
...
def configured_filelist(args: argparse.Namespace, generated_dir: Path) -> FileList:
    names = [
        "def.fl",
        "sys_def.fl",
        "commonip.fl",
...
def write_single_sv(filelist: FileList, output: Path) -> None:
    lines = ["// Generated by syn/tools/export_soc_sources.py", *defines_to_sv(filelist.defines), ""]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--core", default="HAZARD3", choices=("HAZARD3", "PICORV32", "MDD"))
    parser.add_argument("--ip", default="NONE", choices=("NONE", "MDD"))
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="retrosoc-filelists-") as temporary:
        ...
        filelist = configured_filelist(args, generated_dir)
