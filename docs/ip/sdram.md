...
The Mini SoC SDRAM block exposes an AXI4 data port and a separate RIBP register
port. The data path is 32-bit, accepts one transaction at a time, and supports
single-beat, `FIXED`, `INCR`, and legal `WRAP` bursts up to sixteen beats. The
AXI-to-RIBP compatibility engine serializes each beat so the existing
phase-separated SDRAM command engine remains unchanged in timing behavior.
...
| RIBP configuration window | `0x1000D000` - `0x1000DFFF` |
...
Configuration requests enter through `sdram_cfg_ribp`, which is generated by
the SoC RIBP configuration plane. AXI data requests enter `axi4`; an internal
RIBP arbiter gives a held request ownership until its response completes. The
merged RIBP request is then decoded to the register block or SDRAM core. The
global `ip_ribp_wrapper` therefore contains no SDRAM physical instance.

## Register ABI

All registers are 32-bit and naturally aligned. Invalid offsets return
`resp_err`; partial writes are accepted according to `wstrb`.
