...
targets are `bus`, `rib_adapter`, `spill_register`, `ribp2apb`, `sysctrl`,
`pll_rcu`, and `gpio_user`; each uses
...
under backpressure. `spill_register` checks the externally visible occupancy,
ready/valid, flush, and stalled-output contract of the common response buffer;
its deterministic simulation scoreboard separately checks data ordering.
