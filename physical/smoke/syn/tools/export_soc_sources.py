...
HAZARD3_UPDATE_FUNCTION_RE = re.compile(
    r"function\s+\[XLEN-1:0\]\s+update_nonconst\s*;\s*"
    r"input\s+\[XLEN-1:0\]\s+prev\s*;\s*"
    r"input\s+\[XLEN-1:0\]\s+nonconst\s*;\s*"
    r"begin\s*"
    r"update_nonconst\s*=\s*\(wdata_update\s*&\s*nonconst\)\s*\|\s*"
    r"\(prev\s*&\s*~nonconst\)\s*;\s*"
    r"end\s*endfunction"
)
HAZARD3_UPDATE_CALL_RE = re.compile(
    r"update_nonconst\(\s*(?P<previous>[A-Za-z_][A-Za-z0-9_$]*)\s*,\s*"
    r"(?P<writable>[A-Za-z_][A-Za-z0-9_$]*)\s*\)"
)
...
def sanitize_librelane_source(source: str) -> str:
    """Inline the Hazard3 CSR helper that yosys-slang mis-models with hierarchy."""
    source, definitions = HAZARD3_UPDATE_FUNCTION_RE.subn("", source, count=1)
    if definitions != 1:
        raise RuntimeError("expected exactly one Hazard3 update_nonconst definition")

    def inline_update(match: re.Match[str]) -> str:
        previous = match.group("previous")
        writable = match.group("writable")
        return f"((wdata_update & {writable}) | ({previous} & ~{writable}))"

    source, calls = HAZARD3_UPDATE_CALL_RE.subn(inline_update, source)
    if calls != 2 or "update_nonconst" in source:
        raise RuntimeError("expected exactly two Hazard3 update_nonconst calls")
    return source


def write_single_sv(filelist: FileList, output: Path, *, librelane_safe: bool = False) -> None:
    lines = [
    ...
    source = "\n".join(lines) + "\n"
    if librelane_safe:
        source = sanitize_librelane_source(source)
...
def metadata_file(value: str) -> tuple[Path, Path]:
    ...
    if (
        ...
        raise argparse.ArgumentTypeError("metadata must use a safe DESTINATION=SOURCE_FILE mapping")
