...
#let apu-release-reference() = [
  ...
  application distribution. The ordinary PRODUCT profile keeps KWS disabled by default.
  #enum(..data.system_reference.apu_implementation.acceptance_steps.map(r=>r.description))
  Accepted model-load DMA may continue while resource quiesce blocks new jobs; quiesce
  *must not deadlock an accepted load*. The ordered LP steps above are extracted from the
  application calls and their failure checkpoints; they do not imply a new executed test.
