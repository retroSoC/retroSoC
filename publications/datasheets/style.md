...
Separate whole-document presentation changes from content ranges. A repository
link replacing the left footer is one global change spanning the delivered page
count, not a substantive rewrite of every page. Keep navigation changes and
pure page/figure-number shifts distinct. Normalize both old identity footers and
new repository footers when comparing retained prose.
...
body headings, bookmarks and both flat figure/table directories are unaffected.
Compose the title separately from the filler/page portion and measure each
number column with its actual font weight, including wrapped long titles.
...
remain regular: in `(APU, partial)`, only `APU` is bold. Below level one, titles
without parentheses receive no additional emphasis. Apply this formatting to rendered contents
...
instruction families. Blockcell 0.1.0 covers architectural FIFOs, line buffers,
packet/table/control storage, descriptor examples, address windows, linker/load
placement and calculated buffer budgets.

Overview classification, the access matrix and software sequence diagrams keep
their existing CeTZ rendering. Register descriptions keep the existing bit-layout
component and field tables; WaveDrom remains the timing renderer. A register bank,
counter or narrative is not by itself a reason to invent an additional FIFO or
fixed binary format.

The wrappers render every diagram as Figure, with the existing black 9 pt top
caption and figure-directory entry, even when the package internally uses a
native table. Keep Inter/Fira Code, gray/gold fills, final type of at least 9 pt,
and the current body width. Numeric memory offsets, wire bit order, payload widths
and storage capacities must be labeled independently. Address windows are not
physical device capacities; diagrams with compressed gaps are not to scale.

Rivet instruction words use upper/lower 32-bit rows so adjacent two-digit bit
indexes remain readable. Coordinate and text settings are calibrated together
against the rendered PDF; do not solve collisions by making type smaller.
Circuiteria boxes use the shared thin-border appearance; its unmodified wire
renderer uses 1 pt control/signal lines and 1.5 pt data buses. Dashed control and
solid data paths are a local convention. Labels remain horizontal on white
backing, with endpoints attached to declared ports. Other diagram line widths
are unaffected.

`system-reference.json.illustrations` references the reviewed circuit, binary
layout and storage catalogs. The publication adapters verify named instance
bindings and declared signal directions/widths, C member/array offsets, complete
field coverage, protocol ordering, FIFO parameters, ranges and selected linker
placements. Preserve actual integration overrides, such as DMA's per-channel
depth, instead of using a child module's default parameter.

APU opcode values and whole-field zero requirements come from the current encoder/
...
storage figures are drawn once; shared instances link to them and retain their
own address, routing and capability differences. The build compares this inventory
with renderer metadata and rejects missing, duplicate or unknown drawings.
Diagram position records support full-size color/grayscale review; they do not
feed layout decisions back into the document.

Compressed packet/array diagrams must say they are not to scale and retain every
semantic field/range in their data. Show actual word/byte ordering separately
from physical wire slots. Linker VMA/LMA figures show placement, not measured
usage or proof that a replacement CRT performs relocation/BSS initialization.
Budget comparisons do not turn distributed per-row/sample overhead into a
fictitious contiguous padding region.

The cache diagram illustrates software-declared 64-byte CBO granules and buffer
ownership, not independently verified physical cache tag/set/way organization.

Package identity is the pair of name and version. Rivet/circuiteria's CeTZ 0.3.4
coexists with the existing 0.5.2; oxifmt 0.2.1 likewise coexists with 1.0.0.
Circuiteria also requires tidy 0.3.0 at runtime. Validate the complete library
import closure from each entrypoint, excluding standalone third-party manuals.
Preserve package sources and licenses; prepare them only through the dependency
lock and existing setup flow. Normal builds verify caches and never download or
rewrite package sources.

### Vector diagrams and Overview

Use the locked CeTZ implementation for the retained Overview, access matrix and
software workflows. Shared boxes have
