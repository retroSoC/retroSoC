...
Use namespaced imports through `diagram-packages.typ` for the representative
diagram families: bytefield 0.0.8 for DMA TCD and native SD command layouts;
rivet 0.3.1 for APU internal instruction fields/examples; blockcell 0.1.0 for
address windows, software cache-maintenance boundaries and UART FIFO storage;
circuiteria 0.2.1 for the UART0, central DMA and APU internal structure figures.
All other component diagrams keep the existing CeTZ renderer. Do not route
register descriptions through these packages: the existing bit-layout component,
field tables and WaveDrom implementation remain separate and unchanged.
...
`system-reference.json.illustrations` owns reviewed field and connection records.
`diagram_reference.py` checks source bindings, descriptor coverage, SD frame bit
order, APU encoder/RTL slices, FIFO geometry and circuit endpoints/directions.
The data enters the publication manifest with its sources. Encoding examples
are generated by the existing APU tool; they are not execution or codec-pass
evidence. Keep capability and disabled-route qualifications on the APU figure.
