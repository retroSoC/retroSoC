// Tiny-owned layout snapshot of the approved Mini style; Mini is not imported or modified.
#let ink = rgb("#292C31")
#let gold = rgb("#A38442")
...
#let ip(id) = {
  ...
  block(above: rhythm.metadata-before, below: rhythm.metadata-after, breakable: false, sticky: true)[
    ...
    #block(above: rhythm.metadata-before)[
      *Core IRQ bit:* #if irqs.len() == 0 { [No dedicated source.] } else {
...
#let template(body) = {
  set document(title: doc.title, author: doc.author,
    keywords: ("retroSoC", "Tiny", "Gen1", "datasheet", doc.document_id, "v"+doc.version, doc.status))
