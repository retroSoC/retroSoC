...
static uint32_t donut_framebuffer_32[DONUT_WIDTH * DONUT_HEIGHT];
...
//     350, 367, 384, 400, 416, 433, 449, 465, 481, 496, 512, 527, 542, 557, 572, 587, 601, 615,
//     629, 643, 657, 670, 683, 696, 709, 721, 733, 745, 757, 768, 779, 790, 801, 811, 821, 831,
//     841, 850, 859, 868, 876, 884, 892, 900, 907, 914, 921, 927, 933, 939, 945, 950, 955, 960,
//     964, 968, 972, 975, 978, 981, 984, 986, 988, 990, 991, 992, 993, 994, 994, 995, 995
// };

// static const int16_t cos_table[91] = {
//     1024, 1023, 1023, 1022, 1021, 1019, 1017, 1014, 1011, 1007, 1003, 998, 993, 987, 981, 974,
//     967, 959, 951, 942, 933, 923, 913, 902, 891, 879, 867, 854, 841, 827, 813, 798, 783, 767,
//     751, 734, 717, 699, 681, 662, 643, 623, 603, 582, 561, 539, 517, 494, 471, 447, 423, 398,
//     373, 347, 321, 294, 267, 239, 211, 182, 153, 123, 93, 62, 31, 0, -31, -62, -93, -123, -153,
//     -182, -211, -239, -267, -294, -321, -347, -373, -398, -423, -447, -471, -494, -517, -539,
//     -561, -582, -603, -623, -643
// };
...
static void rs_donut_render(uint32_t frame_count) {
    int sA = 1024;
    int cA = 0;
    int sB = 1024;
    int cB = 0;
    ...
    for (uint32_t frame = 0U; frame < frame_count; ++frame) {
        (void)memset(pixel_buffer, 0, sizeof(pixel_buffer));
        (void)memset(z_buffer, 0x7F, sizeof(z_buffer));
        (void)memset(donut_framebuffer_32, 0, sizeof(donut_framebuffer_32));
        ...
        for (int j = 0; j < 90; j++) {
            ...
            for (int i = 0; i < DONUT_SIZE * DONUT_SIZE; i++) {
                ...
                int N = (((-cA * x7 - cB * ((-sA * x7 >> 10) + x2) - ci * (cj * sB >> 10)) >> 10) -
                         x5) >>
                        7;
                ...
                if (likely(y > 0 && y < DONUT_SIZE && x > 0 && x < DONUT_SIZE)) {
                    ...
                    if (likely(zz < z_buffer[o])) {
                        ...
                        const uint32_t shade =
                            (N <= 0) ? 0U : ((N >= GRAY_LEVELS) ? GRAY_LEVELS - 1U : (uint32_t)N);
                        uint32_t color = gray_colors[shade];
...
void donut_test(int argc, char **argv) {
    ...
    printf("donut test\n");
    rs_donut_render(RS_DONUT_FRAME_COUNT);
}
