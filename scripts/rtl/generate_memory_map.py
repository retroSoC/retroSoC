...
MAX_ADDRESS = 1 << 32
VALID_ROUTES = {"apb4_periph", "apb4_system", "axi4", "ram", "reserved"}
...
def render_rtl(
    cpu_reset: int, regions: list[dict[str, Any]], sysctrl_registers: list[dict[str, Any]], legacy_rib: bool = True
) -> str:
    lines = [
        "// Generated by scripts/rtl/generate_memory_map.py; do not edit.",
    ...
    lines.append("")
    for route in ("apb4_periph", "apb4_system", "axi4", "ram", "reserved"):
        selected = [
...
def render_c(
    ...
    if capability_header is not None:
        lines.append(f"#include <{capability_header}>")
    lines += ["", "#endif", ""]
    return "\n".join(lines)
...
def generate(map_path: Path, output_dir: Path, have_sram_if: str, sram_size_kib: int) -> None:
    ...
    atomic_write(
        ...
        render_rtl(cpu_reset, regions, sysctrl_registers,
                   json.loads(map_path.read_text()).get("legacy_rib", True)),
    )
    ...
    atomic_write(
        ...
        render_c(cpu_reset, regions, sysctrl_registers, have_sram_if,
                 json.loads(map_path.read_text()).get("capability_header")),
    )
