...
def render_ports(pads: list[Pad]) -> str:
    lines = [
        "// Generated by scripts/rtl/generate_pin_map.py; do not edit.",
        "`ifdef PDK_IHP130",
...
def render_core_ports(pads: list[Pad]) -> str:
    """Render the logical-chip port list without technology power pins."""
    lines = ["// Generated by scripts/rtl/generate_pin_map.py; do not edit."]
    for index, pad in enumerate(pads):
        suffix = "," if index != len(pads) - 1 else ""
        lines.extend(wrap_feature(pad, f"    {pad.direction} {pad.name}{suffix}"))
    return "\n".join(lines) + "\n"
...
def render_core_binding(pad: Pad) -> str | None:
    """Bind a package-facing core port directly to the logical SoC interface."""
    if pad.kind in {"tri_input", "schmitt_input"}:
        assert pad.signal is not None
        return f"  assign {pad.signal} = {pad.name};"
    if pad.kind in {"tri_output", "peripheral_output"}:
        assert pad.signal is not None
        return f"  assign {pad.name} = {pad.signal};"
    if pad.kind == "peripheral_bidir":
        assert pad.input_signal is not None
        assert pad.output_signal is not None
        assert pad.output_enable_signal is not None
        return "\n".join(
            [
                f"  assign {pad.name} = {pad.output_enable_signal} ? {pad.output_signal} : 1'bz;",
                f"  assign {pad.input_signal} = {pad.name};",
            ]
        )
    if pad.kind == "full":
        assert pad.interface is not None
        assert pad.index is not None
        return "\n".join(
            [
                f"  assign {pad.name} = {pad.interface}.oe_o[{pad.index}] ? "
                f"{pad.interface}.do_o[{pad.index}] : 1'bz;",
                f"  assign {pad.interface}.di_i[{pad.index}] = {pad.name};",
            ]
        )
    if pad.kind == "xpi_data":
        assert pad.index is not None
        return "\n".join(
            [
                f"  assign {pad.name} = u_xpi_if.io_oe_o[{pad.index}] ? "
                f"u_xpi_if.io_do_o[{pad.index}] : 1'bz;",
                f"  assign u_xpi_if.io_di_i[{pad.index}] = {pad.name};",
            ]
        )
    if pad.kind == "sdram_dq":
        assert pad.index is not None
        return "\n".join(
            [
                f"  assign {pad.name} = u_sdram_if.oe_o ? u_sdram_if.dq_o[{pad.index}] : 1'bz;",
                f"  assign u_sdram_if.dq_i[{pad.index}] = {pad.name};",
            ]
        )
    if pad.kind == "xtal" and pad.bind:
        assert pad.signal is not None
        assert pad.peer is not None
        return "\n".join(
            [
                f"  assign {pad.signal} = {pad.name};",
                f"  assign {pad.peer} = 1'b0;",
            ]
        )
    return None


def render_core_bindings(pads: list[Pad]) -> str:
    lines = ["// Generated by scripts/rtl/generate_pin_map.py; do not edit."]
    for pad in pads:
        binding = render_core_binding(pad)
        if binding is not None:
            lines.extend(wrap_feature(pad, binding))
    return "\n".join(lines) + "\n"
