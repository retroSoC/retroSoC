...
typedef union {
    struct {
        ...
        rv_csr_t j : 1; /*!< bit:     9  Tentatively reserved for Dynamically Translated Languages
                           extension */
        rv_csr_t _reserved1 : 1; /*!< bit:     10 Reserved  */
        rv_csr_t
            l : 1; /*!< bit:     11 Tentatively reserved for Decimal Floating-Point extension  */
...
typedef union {
    struct {
        ...
        rv_csr_t
            interrupt : 1; /*!< bit:     63  trap type. 0 means exception and 1 means interrupt */
#else
        rv_csr_t
            interrupt : 1; /*!< bit:     31  trap type. 0 means exception and 1 means interrupt */
#endif
    } b;        /*!< Structure used for bit  access */
    rv_csr_t d; /*!< Type      used for csr data access */
} CSR_MCAUSE_Type;
 ...
 * memory accesses from instructions appearing after the fence in program order (the `successor
 *set`). For details, please refer to The RISC-V Instruction Set Manual \param p     predecessor
 *set, such as iorw, rw, r, w \param s     successor set, such as iorw, rw, r, w
 **/
