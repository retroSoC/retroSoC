...
void ip_psram_selftest(uint32_t addr, uint32_t range) {
    ...
    if ((addr == 0U) || ((addr & (sizeof(uint32_t) - 1U)) != 0U) || (range < sizeof(uint32_t))) {
        printf("[PSRAM] self test range is invalid\n");
        return;
    }
    test_num = range / sizeof(uint32_t);
    if (test_num > 8192U) {
        test_num = 8192U;
    }

    printf("[range: %dB] 4-bytes wr/rd test\n", 4 * test_num);
    for (uint32_t i = 0U; i < test_num; ++i) {
    ...
    const uint32_t cyc_count = 5U;
    const uint32_t stride = 256U;
    uint32_t state;

    printf("[range: %dMB] stride increments wr/rd test\n", range / 1024 / 1024);
    for (uint32_t i = 1U; i <= cyc_count; ++i) {
        state = i;
        for (uint32_t word = 0U; word < range / sizeof(*base_word); word += stride) {
