...
uint32_t i2c1_get_ack() {
    while ((reg_i2c1_sr & I2C1_STATUS_TIP) == 0); // need TIP go to 1
    while ((reg_i2c1_sr & I2C1_STATUS_TIP) != 0); // and then go back to 0
    return !(reg_i2c1_sr & I2C1_STATUS_RXACK);    // invert since signal is active low
}

uint32_t i2c1_busy() {
    return ((reg_i2c1_sr & I2C1_STATUS_BUSY) == I2C1_STATUS_BUSY);
}

void i2c1_wr_start(uint32_t slv_addr) {
    reg_i2c1_txr = slv_addr;
    reg_i2c1_cmd = I2C1_START_WRITE;
    if (!i2c1_get_ack()) printf("[wr start]no ack recv\n");
}
...
void i2c1_write(uint8_t val) {
    reg_i2c1_txr = val;
    reg_i2c1_cmd = I2C1_WRITE;
    if (!i2c1_get_ack()) printf("[i2c write]no ack recv\n");
    // do {
    //     reg_i2c1_txr = val;
    //     reg_i2c1_cmd = I2C1_WRITE;
    // } while(!i2c_get_ack());
...
uint32_t i2c1_read(uint32_t cmd) {
    reg_i2c1_cmd = cmd;
    if (!i2c1_get_ack()) printf("[i2c read]no ack recv\n");
    return reg_i2c1_rxr;
}

void i2c1_stop() {
    reg_i2c1_cmd = I2C1_STOP;
    while(i2c1_busy());
}

void i2c1_wr_nbyte(uint8_t slv_addr, uint16_t reg_addr, uint8_t type, uint8_t num, uint8_t *data) {
    // i2c_wr_start(slv_addr);
    i2c1_rd_start(slv_addr);
    if(type == I2C_DEV_ADDR_16BIT) {
        i2c1_write((uint8_t)((reg_addr >> 8) & 0xFF));
        i2c1_write((uint8_t)(reg_addr & 0xFF));
    } else {
        i2c1_write((uint8_t)(reg_addr & 0xFF));
    }
    for(int i = 0; i < num; ++i) {
...
void i2c1_rd_nbyte(uint8_t slv_addr, uint16_t reg_addr, uint8_t type, uint8_t num, uint8_t *data) {
    ...
    i2c1_wr_start(slv_addr + 1);
    for (int i = 0; i < num; ++i) {
        if (i == num - 1) data[i] = i2c1_read(I2C1_STOP_READ);
        else data[i] = i2c1_read(I2C1_READ);
    }
}