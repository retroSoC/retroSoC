...
module axi4l2rib (
  ...
  always_comb begin
    ...
    case (s_rd_fsm_q)
      ...
      RD_DATA: begin
        if (rib.ready) s_rd_fsm_d = RD_WAIT;
      end
      RD_WAIT: begin
  ...
  always_comb begin
    ...
    case (s_wr_fsm_q)
      ...
      WR_WAIT: begin
        if (rib.ready) s_wr_fsm_d = WR_RESP;
      end
      WR_RESP: begin
  ...
  assign s_rdata_d = rib.rdata;
  dffer #(32) u_rdata_dffer (
      aclk_i,
      aresetn_i,
      s_rd_fsm_q == RD_DATA && rib.ready,
      s_rdata_d,
      s_rdata_q
  );
  ...
  assign rib.valid = s_rd_req || s_wr_req;
  assign rib.wdata = s_wdata_q;
  assign rib.wstrb = (s_wr_fsm_q == WR_WAIT) ? s_wstrb_q : '0;
endmodule
