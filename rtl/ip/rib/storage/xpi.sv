...
module rib_xpi (
  ...
  // rib mux
  assign s_xpi_mm_sel       = `SOC_ADDR_IS_FLASH(rib.addr) || `SOC_ADDR_IS_XPI(rib.addr);
  assign u_reg_rib_if.valid = rib.valid && (~s_xpi_mm_sel);
  assign u_reg_rib_if.addr  = rib.addr;
  assign u_reg_rib_if.wdata = rib.wdata;
  assign u_reg_rib_if.wstrb = rib.wstrb;

  assign u_mm_rib_if.valid  = rib.valid && s_xpi_mm_sel;
  assign u_mm_rib_if.addr   = rib.addr;
  assign u_mm_rib_if.wdata  = rib.wdata;
  assign u_mm_rib_if.wstrb  = rib.wstrb;

  // verilog_format: off
  assign rib.ready = (u_reg_rib_if.valid & u_reg_rib_if.ready) |
                     (u_mm_rib_if.valid  & u_mm_rib_if.ready);

  assign rib.rdata = ({32{(u_reg_rib_if.valid & u_reg_rib_if.ready)}} & u_reg_rib_if.rdata) |
                     ({32{(u_mm_rib_if.valid  & u_mm_rib_if.ready)}}  & u_mm_rib_if.rdata);
  // verilog_format: on
  ...
  xpi_core u_xpi_core (
      ...
      .rdwr_i(u_mm_rib_if.valid ? s_xpi_mm_rdwr : s_xpi_rdwr[s_xpi_nss]),
      ...
      // HACK:
      .adrdat_i(u_mm_rib_if.valid ? {s_xpi_mm_addr[23:0], 8'hF0} : s_xpi_adrdat[s_xpi_nss]),
      ...
      .datbit_i(u_mm_rib_if.valid ? s_xpi_mm_xfer_byte : s_xpi_datbit[s_xpi_nss]),
      ...
      .start_i(u_mm_rib_if.valid ? s_xpi_mm_req : s_reg_xfer_start),
      .done_o(s_xfer_done),
