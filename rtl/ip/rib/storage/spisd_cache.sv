...
module spisd_cache (
  ...
  assign rib.rdata       = s_cache_data_q[s_cache_index];
  // cache
  assign s_cache_index   = rib.addr[8:2];
  assign s_cache_mem_hit = ~mode_i && (rib.addr[27:9] == s_cache_tag_q[18:0]);
  assign s_cache_byp_hit = mode_i && (rib.addr == s_cache_tag_q);
    ...
    unique case (s_cache_fsm_q)
      FSM_IDLE: begin
        if (init_done_i) begin
          if (rib.valid) s_cache_fsm_d = FSM_COMP_TAG;
          // sw wr sync
      ...
      FSM_COMP_TAG: begin
        ...
        if ((s_cache_mem_hit || s_cache_byp_hit) && s_cache_valid_q) begin
          if (s_wr_sync_q) wr_sync_done_o = 1'b1;  // wr sync oper
            ...
            rib.ready = 1'b1;  // nomral oper
            // write oper, set dirty
            if (|rib.wstrb) begin
              s_cache_dirty_d = 1'b1;
              if (rib.wstrb[0]) s_cache_data_d[s_cache_index][7:0] = rib.wdata[7:0];
              if (rib.wstrb[1]) s_cache_data_d[s_cache_index][15:8] = rib.wdata[15:8];
              if (rib.wstrb[2]) s_cache_data_d[s_cache_index][23:16] = rib.wdata[23:16];
              if (rib.wstrb[3]) s_cache_data_d[s_cache_index][31:24] = rib.wdata[31:24];
            end
          end
          ...
          s_cache_dirty_d = |rib.wstrb;
          // tag line is clean
          if (s_cache_valid_q == 1'b0 || s_cache_dirty_q == 1'b0) begin
            s_cache_fsm_d = FSM_ALLOC;
            if (~mode_i) s_cache_tag_d = {13'd0, rib.addr[27:9]};
            else s_cache_tag_d = rib.addr;
          end else begin
