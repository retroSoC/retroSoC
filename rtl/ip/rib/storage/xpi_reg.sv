...
module xpi_reg (
  ...
  assign s_xpi_cfgidx_en = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_CFGIDX;
  assign s_xpi_cfgidx_d  = rib.wdata[`XPI_LNS_NUM-1:0];
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ACCMD_BLOCK
    assign s_xpi_accmd_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ACCMD && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_accmd_d[i] = rib.wdata[0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_MMSTAD_BLOCK
    assign s_xpi_mmstad_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_MMSTAD && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    always_comb begin
      s_xpi_mmstad_d[i] = s_xpi_mmstad_q[i];
      if (rib.wstrb[0]) s_xpi_mmstad_d[i][7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_xpi_mmstad_d[i][15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_xpi_mmstad_d[i][23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_xpi_mmstad_d[i][31:24] = rib.wdata[31:24];
    end
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_MMOFFST_BLOCK
    assign s_xpi_mmoffst_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_MMOFFST && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    always_comb begin
      s_xpi_mmoffst_d[i] = s_xpi_mmoffst_q[i];
      if (rib.wstrb[0]) s_xpi_mmoffst_d[i][7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_xpi_mmoffst_d[i][15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_xpi_mmoffst_d[i][23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_xpi_mmoffst_d[i][31:24] = rib.wdata[31:24];
    end
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_MODE_BLOCK
    assign s_xpi_mode_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_MODE && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_mode_d[i] = rib.wdata[0];
  end
  ...
  assign s_xpi_nss_en = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_NSS;
  assign s_xpi_nss_d  = rib.wdata[`XPI_LNS_NUM-1:0];
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_CLKDIV_BLOCK
    assign s_xpi_clkdiv_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_CLKDIV && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_clkdiv_d[i] = rib.wdata[7:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_RDWR_BLOCK
    assign s_xpi_rdwr_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_RDWR && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_rdwr_d[i] = rib.wdata[0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_REVDAT_BLOCK
    assign s_xpi_revdat_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_REVDAT && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_revdat_d[i] = rib.wdata[0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_TXUPB_BLOCK
    assign s_xpi_txupb_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_TXUPB && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_txupb_d[i] = rib.wdata[7:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_TXLOWB_BLOCK
    assign s_xpi_txlowb_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_TXLOWB && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_txlowb_d[i] = rib.wdata[7:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_RXUPB_BLOCK
    assign s_xpi_rxupb_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_RXUPB && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_rxupb_d[i] = rib.wdata[5:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_RXLOWB_BLOCK
    assign s_xpi_rxlowb_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_RXLOWB && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_rxlowb_d[i] = rib.wdata[5:0];
  end
  ...
  assign s_xpi_flush     = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_FLUSH;
  assign s_xpi_flush_val = rib.wdata[0];


  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_CMDTYP_BLOCK
    assign s_xpi_cmdtyp_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_CMDTYP && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_cmdtyp_d[i] = rib.wdata[1:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_CMDLEN_BLOCK
    assign s_xpi_cmdlen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_CMDLEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_cmdlen_d[i] = rib.wdata[2:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_CMDDAT_BLOCK
    assign s_xpi_cmddat_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_CMDDAT && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    always_comb begin
      s_xpi_cmddat_d[i] = s_xpi_cmddat_q[i];
      if (rib.wstrb[0]) s_xpi_cmddat_d[i][7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_xpi_cmddat_d[i][15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_xpi_cmddat_d[i][23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_xpi_cmddat_d[i][31:24] = rib.wdata[31:24];
    end
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ADRTYP_BLOCK
    assign s_xpi_adrtyp_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ADRTYP && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_adrtyp_d[i] = rib.wdata[1:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ADRLEN_BLOCK
    assign s_xpi_adrlen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ADRLEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_adrlen_d[i] = rib.wdata[2:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ADRDAT_BLOCK
    assign s_xpi_adrdat_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ADRDAT && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    always_comb begin
      s_xpi_adrdat_d[i] = s_xpi_adrdat_q[i];
      if (rib.wstrb[0]) s_xpi_adrdat_d[i][7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_xpi_adrdat_d[i][15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_xpi_adrdat_d[i][23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_xpi_adrdat_d[i][31:24] = rib.wdata[31:24];
    end
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ALTTYP_BLOCK
    assign s_xpi_alttyp_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ALTTYP && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_alttyp_d[i] = rib.wdata[1:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ALTLEN_BLOCK
    assign s_xpi_altlen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ALTLEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_altlen_d[i] = rib.wdata[2:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_ALTDAT_BLOCK
    assign s_xpi_altdat_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_ALTDAT && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    always_comb begin
      s_xpi_altdat_d[i] = s_xpi_altdat_q[i];
      if (rib.wstrb[0]) s_xpi_altdat_d[i][7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_xpi_altdat_d[i][15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_xpi_altdat_d[i][23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_xpi_altdat_d[i][31:24] = rib.wdata[31:24];
    end
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_TDULEN_BLOCK
    assign s_xpi_tdulen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_TDULEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_tdulen_d[i] = rib.wdata[7:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_RDULEN_BLOCK
    assign s_xpi_rdulen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_RDULEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_rdulen_d[i] = rib.wdata[7:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_DATTYP_BLOCK
    assign s_xpi_dattyp_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_DATTYP && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_dattyp_d[i] = rib.wdata[1:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_DATLEN_BLOCK
    assign s_xpi_datlen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_DATLEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_datlen_d[i] = rib.wdata[7:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_DATBIT_BLOCK
    assign s_xpi_datbit_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_DATBIT && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_datbit_d[i] = rib.wdata[2:0];
  end
  ...
  for (genvar i = 0; i < `XPI_NSS_NUM; i++) begin : XPI_HLVLEN_BLOCK
    assign s_xpi_hlvlen_en[i] = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_HLVLEN && `XPI_LNS_NUM'(i) == s_xpi_cfgidx_q;
    assign s_xpi_hlvlen_d[i] = rib.wdata[7:0];
  end
  ...
  always_comb begin
    ...
    tx_push_data_o  = '0;
    if (s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_TXDATA) begin
      tx_push_valid_o = 1'b1;
      if (rib.wstrb[0]) tx_push_data_o[7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) tx_push_data_o[15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) tx_push_data_o[23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) tx_push_data_o[31:24] = rib.wdata[31:24];
    end
  end

  // start
  assign xfer_start_o = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_XPI_XST;
  ...
  always_comb begin
    ...
    end else if (s_rib_rd_hdshk && rib.addr[7:0] == `RIB_XPI_STATUS) begin
  ...
  always_comb begin
    ...
    unique case (rib.addr[7:0])
      `RIB_XPI_CFGIDX:  s_rib_rdata_d = {{(32 - `XPI_LNS_NUM) {1'b0}}, s_xpi_cfgidx_q};
      `RIB_XPI_ACCMD:   s_rib_rdata_d = {31'd0, s_xpi_accmd_q[s_xpi_cfgidx_q]};
      `RIB_XPI_MMSTAD:  s_rib_rdata_d = s_xpi_mmstad_q[s_xpi_cfgidx_q];
      `RIB_XPI_MMOFFST: s_rib_rdata_d = s_xpi_mmoffst_q[s_xpi_cfgidx_q];
      `RIB_XPI_MODE:    s_rib_rdata_d = {31'd0, s_xpi_mode_q[s_xpi_cfgidx_q]};
      `RIB_XPI_NSS:     s_rib_rdata_d = {{(32 - `XPI_LNS_NUM) {1'b0}}, s_xpi_nss_q};
      `RIB_XPI_CLKDIV:  s_rib_rdata_d = {24'd0, s_xpi_clkdiv_q[s_xpi_cfgidx_q]};
      `RIB_XPI_RDWR:    s_rib_rdata_d = {31'd0, s_xpi_rdwr_q[s_xpi_cfgidx_q]};
      `RIB_XPI_REVDAT:  s_rib_rdata_d = {31'd0, s_xpi_revdat_q[s_xpi_cfgidx_q]};
      `RIB_XPI_TXUPB:   s_rib_rdata_d = {24'd0, s_xpi_txupb_q[s_xpi_cfgidx_q]};
      `RIB_XPI_TXLOWB:  s_rib_rdata_d = {24'd0, s_xpi_txlowb_q[s_xpi_cfgidx_q]};
      `RIB_XPI_RXUPB:   s_rib_rdata_d = {26'd0, s_xpi_rxupb_q[s_xpi_cfgidx_q]};
      `RIB_XPI_RXLOWB:  s_rib_rdata_d = {26'd0, s_xpi_rxlowb_q[s_xpi_cfgidx_q]};
      `RIB_XPI_CMDTYP:  s_rib_rdata_d = {30'd0, s_xpi_cmdtyp_q[s_xpi_cfgidx_q]};
      `RIB_XPI_CMDLEN:  s_rib_rdata_d = {29'd0, s_xpi_cmdlen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_CMDDAT:  s_rib_rdata_d = s_xpi_cmddat_q[s_xpi_cfgidx_q];
      `RIB_XPI_ADRTYP:  s_rib_rdata_d = {30'd0, s_xpi_adrtyp_q[s_xpi_cfgidx_q]};
      `RIB_XPI_ADRLEN:  s_rib_rdata_d = {29'd0, s_xpi_adrlen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_ADRDAT:  s_rib_rdata_d = s_xpi_adrdat_q[s_xpi_cfgidx_q];
      `RIB_XPI_ALTTYP:  s_rib_rdata_d = {30'd0, s_xpi_alttyp_q[s_xpi_cfgidx_q]};
      `RIB_XPI_ALTLEN:  s_rib_rdata_d = {29'd0, s_xpi_altlen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_ALTDAT:  s_rib_rdata_d = s_xpi_altdat_q[s_xpi_cfgidx_q];
      `RIB_XPI_TDULEN:  s_rib_rdata_d = {24'd0, s_xpi_tdulen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_RDULEN:  s_rib_rdata_d = {24'd0, s_xpi_rdulen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_DATTYP:  s_rib_rdata_d = {30'd0, s_xpi_dattyp_q[s_xpi_cfgidx_q]};
      `RIB_XPI_DATLEN:  s_rib_rdata_d = {24'd0, s_xpi_datlen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_DATBIT:  s_rib_rdata_d = {29'd0, s_xpi_datbit_q[s_xpi_cfgidx_q]};
      `RIB_XPI_HLVLEN:  s_rib_rdata_d = {24'd0, s_xpi_hlvlen_q[s_xpi_cfgidx_q]};
      `RIB_XPI_RXDATA: begin
        if (s_rib_rd_hdshk) begin
          rx_pop_valid_o = 1'b1;
          if (~rx_empty_i) s_rib_rdata_d = rx_pop_data_i;
          else s_rib_rdata_d = '0;
        end
      end
      `RIB_XPI_STATUS:  s_rib_rdata_d = {11'd0, s_xpi_status_q};
