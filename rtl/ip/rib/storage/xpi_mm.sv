...
module xpi_mm (
  ...
  // rib rd
  always_comb begin
    if (s_rib_rd_hdshk) s_rib_ready_d = rx_pop_ready_i;
    else if (s_rib_wr_hdshk) s_rib_ready_d = xfer_done_i;
    else s_rib_ready_d = 1'b0;
  end
  ...
  assign rx_pop_valid_o = s_rib_rd_hdshk;
  assign s_rib_rdata_en = s_rib_rd_hdshk;
  assign s_rib_rdata_d  = (rib.valid && rx_pop_ready_i) ? rx_pop_data_i : s_rib_rdata_q;
  ...
  always_comb begin
    ...
    for (int i = 0; i < `XPI_NSS_NUM; i++) begin
      if (xpi_mmstad_i[i] <= rib.addr && rib.addr <= xpi_mmstad_i[i] + xpi_mmoffst_i[i]) begin
        ...
        FSM_IDLE: begin
          if (s_mem_valid_re) begin
            if (|rib.wstrb) begin
              ...
              r_addr          <= {4'd0, rib.addr[27:0]} + {30'd0, s_disp_addr_ofst};
              r_wr_data       <= s_disp_wdata;
              r_xfer_byte_cnt <= s_disp_byte_cnt;
            end else begin
              r_fsm_state     <= FSM_RD_ST;
              r_addr          <= {4'd0, rib.addr[27:0]};
              r_wr_data       <= rib.wdata;  // NOTE: no used
