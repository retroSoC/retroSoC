...
// verilog_format: off
`define RIB_CLINT_MTIMEL    8'h00
`define RIB_CLINT_MTIMEH    8'h04
`define RIB_CLINT_MTIMECMPL 8'h08
`define RIB_CLINT_MTIMECMPH 8'h0C
`define RIB_CLINT_MSIP      8'h10
`define RIB_CLINT_CLKDIV    8'h14
// verilog_format: on
...
module rib_clint (
  ...
  assign s_clint_clkdiv_en = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_CLINT_CLKDIV;
  assign s_clint_clkdiv_d  = rib.wdata[7:0];
  ...
  assign s_clint_mtimecmp_en = s_rib_wr_hdshk && (rib.addr[7:0] == `RIB_CLINT_MTIMECMPL || rib.addr[7:0] == `RIB_CLINT_MTIMECMPH);
  always_comb begin
    s_clint_mtimecmp_d = s_clint_mtimecmp_q;
    if (rib.addr[7:0] == `RIB_CLINT_MTIMECMPL) begin
      if (rib.wstrb[0]) s_clint_mtimecmp_d[7:0] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_clint_mtimecmp_d[15:8] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_clint_mtimecmp_d[23:16] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_clint_mtimecmp_d[31:24] = rib.wdata[31:24];
    end else if (rib.addr[7:0] == `RIB_CLINT_MTIMECMPH) begin
      if (rib.wstrb[0]) s_clint_mtimecmp_d[39:32] = rib.wdata[7:0];
      if (rib.wstrb[1]) s_clint_mtimecmp_d[47:40] = rib.wdata[15:8];
      if (rib.wstrb[2]) s_clint_mtimecmp_d[55:48] = rib.wdata[23:16];
      if (rib.wstrb[3]) s_clint_mtimecmp_d[63:56] = rib.wdata[31:24];
    end
  end
  ...
  assign s_clint_msip_en = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_CLINT_MSIP;
  assign s_clint_msip_d  = rib.wdata[0];
  ...
  always_comb begin
    ...
    unique case (rib.addr[7:0])
      `RIB_CLINT_CLKDIV:    s_rib_rdata_d = {24'd0, s_clint_clkdiv_q};
      `RIB_CLINT_MTIMEL:    s_rib_rdata_d = s_clint_mtime_q[31:0];
      `RIB_CLINT_MTIMEH:    s_rib_rdata_d = s_clint_mtime_q[63:32];
      `RIB_CLINT_MTIMECMPL: s_rib_rdata_d = s_clint_mtimecmp_q[31:0];
      `RIB_CLINT_MTIMECMPH: s_rib_rdata_d = s_clint_mtimecmp_q[63:32];
      `RIB_CLINT_MSIP:      s_rib_rdata_d = {31'd0, s_clint_msip_q};
