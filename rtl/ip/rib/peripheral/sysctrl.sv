...
module rib_sysctrl (
  ...
  localparam sysctrl_offset_t SYSCTRL_PERF_RIB_WAIT_LO_OFFSET =
      sysctrl_offset_t'(`SOC_SYSCTRL_PERF_RIB_WAIT_LO_OFFSET);
  localparam sysctrl_offset_t SYSCTRL_PERF_RIB_WAIT_HI_OFFSET =
      sysctrl_offset_t'(`SOC_SYSCTRL_PERF_RIB_WAIT_HI_OFFSET);
  ...
  assign s_sysctrl_coresel_write_valid = rib.wdata[`USER_CORESEL_WIDTH-1:0] < `USER_CORE_COUNT;
  assign s_sysctrl_coresel_en = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_CORESEL_OFFSET &&
                              rib.wstrb[0] && s_sysctrl_coresel_write_valid &&
                              (&s_user_reset_q) && sysctrl.user_bus_idle_i && ~s_user_running_q;
  assign s_sysctrl_coresel_d = rib.wdata[`USER_CORESEL_WIDTH-1:0];
  ...
  assign s_user_reset_write_data = rib.wdata[`USER_CORE_COUNT-1:0];
  ...
  assign s_user_reset_en = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_USER_CORE_RESET_OFFSET &&
                           rib.wstrb[0];
  assign s_user_config_error_clear = s_rib_wr_hdshk &&
      rib.addr[7:0] == SYSCTRL_USER_CORE_STATUS_OFFSET && rib.wstrb[1] && rib.wdata[11];
  ...
  always_comb begin
    ...
    if (s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_CORESEL_OFFSET && ~s_sysctrl_coresel_en) begin
  ...
  assign s_sysctrl_ipsel_en = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_IPSEL_OFFSET;
  assign s_sysctrl_ipsel_d  = rib.wdata[`USER_IPSEL_WIDTH-1:0];
  ...
  assign s_pll_cfg_en = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_PLL_CFG_OFFSET && rib.wstrb[0];
  assign s_pll_cfg_d  = rib.wdata[2:0];
  ...
  assign s_pll_apply = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_PLL_CMD_OFFSET &&
                       rib.wstrb[0] && rib.wdata[0];
  assign s_pll_clear_error = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_PLL_CMD_OFFSET &&
                             rib.wstrb[0] && rib.wdata[1];
  ...
  assign s_perf_enable_en = s_rib_wr_hdshk && rib.addr[7:0] == SYSCTRL_PERF_CTRL_OFFSET &&
                            rib.wstrb[0];
  assign s_perf_enable_d = rib.wdata[0];
  assign s_perf_clear = s_perf_enable_en && rib.wdata[1];
  assign s_perf_snapshot = s_perf_enable_en && rib.wdata[2];
  ...
  dffer #(64) u_perf_rib_wait_dffer (
      clk_i,
      rst_n_i,
      s_perf_snapshot,
      sysctrl.perf_rib_wait_i,
      s_perf_rib_wait_q
  );
  ...
  assign s_fault_status_clear = s_rib_wr_hdshk &&
                                rib.addr[7:0] == SYSCTRL_FAULT_STATUS_OFFSET &&
                                rib.wstrb[0] && rib.wdata[0];
  always_comb begin
  ...
  always_comb begin
    ...
    unique case (rib.addr[7:0])
      // verilog_format: off
      SYSCTRL_CORESEL_OFFSET:            s_rib_rdata_d = {{(32 - `USER_CORESEL_WIDTH) {1'b0}}, s_sysctrl_coresel_q};
      SYSCTRL_IPSEL_OFFSET:              s_rib_rdata_d = {{(32 - `USER_IPSEL_WIDTH) {1'b0}}, s_sysctrl_ipsel_q};
      SYSCTRL_PLL_CFG_OFFSET:            s_rib_rdata_d = {29'd0, s_pll_cfg_q};
      SYSCTRL_FAULT_STATUS_OFFSET:       s_rib_rdata_d = {27'd0, s_fault_reason_q, s_fault_write_q, s_fault_pending_q};
      SYSCTRL_FAULT_ADDR_OFFSET:         s_rib_rdata_d = s_fault_addr_q;
      SYSCTRL_FAULT_COUNT_OFFSET:        s_rib_rdata_d = s_fault_count_q;
      SYSCTRL_PLL_STATUS_OFFSET:         s_rib_rdata_d = {
          21'd0,
          pll_ctrl.capable_i,
          s_pll_lock_q,
          s_pll_safe_clk_q,
      ...
      SYSCTRL_USER_CORE_RESET_OFFSET:    s_rib_rdata_d = {{(32 - `USER_CORE_COUNT) {1'b0}}, s_user_reset_q};
      SYSCTRL_USER_CORE_STATUS_OFFSET:   s_rib_rdata_d = {
          20'd0,
          s_user_config_error_q,
          s_user_draining_q,
      ...
      SYSCTRL_FAULT_MASTER_OFFSET:       s_rib_rdata_d = {30'd0, s_fault_master_q};
      SYSCTRL_FAULT_DETAIL_OFFSET:       s_rib_rdata_d = {29'd0, s_fault_detail_q};
      SYSCTRL_PERF_CTRL_OFFSET:          s_rib_rdata_d = {31'd0, s_perf_enable_q};
      SYSCTRL_PERF_MGMT_WAIT_LO_OFFSET:  s_rib_rdata_d = s_perf_mgmt_wait_q[31:0];
      SYSCTRL_PERF_MGMT_WAIT_HI_OFFSET:  s_rib_rdata_d = s_perf_mgmt_wait_q[63:32];
      SYSCTRL_PERF_USER_WAIT_LO_OFFSET:  s_rib_rdata_d = s_perf_user_wait_q[31:0];
      SYSCTRL_PERF_USER_WAIT_HI_OFFSET:  s_rib_rdata_d = s_perf_user_wait_q[63:32];
      SYSCTRL_PERF_DMA_WAIT_LO_OFFSET:   s_rib_rdata_d = s_perf_dma_wait_q[31:0];
      SYSCTRL_PERF_DMA_WAIT_HI_OFFSET:   s_rib_rdata_d = s_perf_dma_wait_q[63:32];
      SYSCTRL_PERF_RIB_WAIT_LO_OFFSET:   s_rib_rdata_d = s_perf_rib_wait_q[31:0];
      SYSCTRL_PERF_RIB_WAIT_HI_OFFSET:   s_rib_rdata_d = s_perf_rib_wait_q[63:32];
      SYSCTRL_PERF_APB_WAIT_LO_OFFSET:   s_rib_rdata_d = s_perf_apb_wait_q[31:0];
      SYSCTRL_PERF_APB_WAIT_HI_OFFSET:   s_rib_rdata_d = s_perf_apb_wait_q[63:32];
      SYSCTRL_PERF_SDRAM_WAIT_LO_OFFSET: s_rib_rdata_d = s_perf_sdram_wait_q[31:0];
      SYSCTRL_PERF_SDRAM_WAIT_HI_OFFSET: s_rib_rdata_d = s_perf_sdram_wait_q[63:32];
      SYSCTRL_PERF_PSRAM_WAIT_LO_OFFSET: s_rib_rdata_d = s_perf_psram_wait_q[31:0];
      SYSCTRL_PERF_PSRAM_WAIT_HI_OFFSET: s_rib_rdata_d = s_perf_psram_wait_q[63:32];
      SYSCTRL_PERF_FLASH_WAIT_LO_OFFSET: s_rib_rdata_d = s_perf_flash_wait_q[31:0];
      SYSCTRL_PERF_FLASH_WAIT_HI_OFFSET: s_rib_rdata_d = s_perf_flash_wait_q[63:32];
