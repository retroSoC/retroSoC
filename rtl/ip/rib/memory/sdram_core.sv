...
`endif
  ...
  logic [24:0] s_rib_addr_d, s_rib_addr_q;  // addr[24:0] (relevant bits)
  logic [31:0] s_rib_wdata_d, s_rib_wdata_q;
  logic [3:0] s_rib_wstrb_d, s_rib_wstrb_q;


  // rib
  assign rib.ready                                                  = s_ready_q;
  ...
  always_comb begin
    ...
    case (s_state_q)
      ...
      IDLE: begin
        ...
        s_dqm_d   = 2'b11;
        s_ready_d = 1'b0;
        if (rib.valid && !s_ready_q) begin
          // Capture RIB inputs into holding registers
          s_rib_addr_d  = rib.addr[24:0];
          s_rib_wdata_d = rib.wdata;
          s_rib_wstrb_d = rib.wstrb;
          s_cmd_d       = CMD_ACT;
          s_ba_d        = rib.addr[22:21];
          s_addr_d      = {rib.addr[24:23], rib.addr[20:10]};
          s_state_d     = WAIT_STATE;
          s_ret_state_d = |rib.wstrb ? COL_WRITEL : COL_READ;
      ...
      COL_READ: begin
        ...
        s_ba_d        = s_rib_addr_q[22:21];
        s_addr_d      = {3'b001, s_rib_addr_q[10:2], 1'b0};
      ...
      COL_WRITEL: begin
        s_cmd_d   = CMD_WRITE;
        s_dqm_d   = ~s_rib_wstrb_q[1:0];
        // autoprecharge and column (use registered addr)
        s_ba_d    = s_rib_addr_q[22:21];
        s_addr_d  = {3'b001, s_rib_addr_q[10:2], 1'b0};
        s_dq_d    = s_rib_wdata_q[15:0];
        s_oe_d    = 1'b1;
        s_state_d = COL_WRITEH;
      end
      COL_WRITEH: begin
        s_cmd_d       = CMD_NOP;
        s_dqm_d       = ~s_rib_wstrb_q[3:2];
        // autoprecharge and column (use registered wdata)
        s_dq_d        = s_rib_wdata_q[31:16];
        s_oe_d        = 1'b1;
        s_state_d     = WAIT_STATE;
        s_ret_state_d = IDLE;
        s_wait_cnt_d  = 16'(TRP);
  ...
  always_ff @(posedge clk_i, negedge rst_n_i) begin
    if (~rst_n_i) begin
      ...
      if (sec_edge_i) begin
        ...
        s_rib_addr_q  <= s_rib_addr_d;
        s_rib_wdata_q <= s_rib_wdata_d;
        s_rib_wstrb_q <= s_rib_wstrb_d;
      end
    end
  end
