...
`ifndef RIB_PSRAM_DEF_SV
`define RIB_PSRAM_DEF_SV

// verilog_format: off
`define RIB_PSRAM_WAIT 8'h00
`define RIB_PSRAM_CHD  8'h04
`define RIB_PSRAM_INIT 8'h08
// verilog_format: on
...
module rib_psram (
  ...
  // verilog_format: off
  assign psram.nss_o[0]   = (~s_init_done) || (s_init_done && rib.addr[24:23] == 2'd0) ? s_psram_ce : 1'b1;
  assign psram.nss_o[1]   = (~s_init_done) || (s_init_done && rib.addr[24:23] == 2'd1) ? s_psram_ce : 1'b1;
  assign psram.nss_o[2]   = (~s_init_done) || (s_init_done && rib.addr[24:23] == 2'd2) ? s_psram_ce : 1'b1;
  assign psram.nss_o[3]   = (~s_init_done) || (s_init_done && rib.addr[24:23] == 2'd3) ? s_psram_ce : 1'b1;
  ...
  assign s_mem_sel     = `SOC_ADDR_IS_PSRAM(rib.addr);
  assign s_cfg_reg_sel = `SOC_ADDR_IS_RIB_PSRAM(rib.addr);
  assign rib.ready     = s_mem_sel ? s_mem_ready : 1'b1;
  always_comb begin
    rib.rdata = '0;
    if (s_mem_sel) begin
      rib.rdata = s_mem_rdata;
    end else if (rib.addr[7:0] == `RIB_PSRAM_WAIT) begin
      rib.rdata = {27'd0, r_cfg_wait};
    end else if (rib.addr[7:0] == `RIB_PSRAM_CHD) begin
      rib.rdata = {29'd0, r_cfg_chd};
    end else if (rib.addr[7:0] == `RIB_PSRAM_INIT) begin
      rib.rdata = {31'd0, r_cfg_init};
    end
  end
  ...
  always_ff @(posedge clk_i, negedge rst_n_i) begin
    ...
    else if (rib.valid && rib.wstrb[0] && s_cfg_reg_sel && rib.addr[7:0] == `RIB_PSRAM_WAIT) begin
      r_cfg_wait <= rib.wdata[4:0];
    end
  end
  // extra cycle for tCHD(mmio)
  always_ff @(posedge clk_i, negedge rst_n_i) begin
    if (~rst_n_i) r_cfg_chd <= 3'd4;
    else if (rib.valid && rib.wstrb[0] && s_cfg_reg_sel && rib.addr[7:0] == `RIB_PSRAM_CHD) begin
      r_cfg_chd <= rib.wdata[2:0];
    end
  end
  // init device/switch qpi mode
  always_ff @(posedge clk_i, negedge rst_n_i) begin
    if (~rst_n_i) r_cfg_init <= '0;
    else if (rib.valid && rib.wstrb[0] && s_cfg_reg_sel && rib.addr[7:0] == `RIB_PSRAM_INIT) begin
      r_cfg_init <= rib.wdata[0];
    end
  end
  ...
  always_ff @(posedge clk_i, negedge rst_n_i) begin
    if (~rst_n_i) begin
      ...
      if (s_mem_sel) begin
        ...
        case (r_fsm_state)
          FSM_IDLE: begin
            if (s_mem_valid_re && (|rib.wstrb)) begin
              r_fsm_state         <= FSM_WE_ST;
              r_xfer_data_bit_cnt <= s_disp_xfer_bit_cnt;
              r_mem_addr          <= {1'b0, rib.addr[22:0]} + {22'd0, s_disp_addr_ofst};
              r_mem_wdata         <= s_disp_wdata;
            end else if (s_mem_valid_re && (~(|rib.wstrb))) begin
              r_fsm_state         <= FSM_RD_ST;
              r_xfer_data_bit_cnt <= 8'd32;
              r_mem_addr          <= {1'b0, rib.addr[22:0]};
              r_mem_wdata         <= rib.wdata;  // NOTE: no used
            end
          end
          FSM_WE_ST: begin
