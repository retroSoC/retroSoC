...
`ifndef RIB_DVP_DEF_SV
`define RIB_DVP_DEF_SV

`define RIB_DVP_RECVEN 8'h00
`define RIB_DVP_RXDATA 8'h04
`define RIB_DVP_STATUS 8'h08
...
module rib_dvp (
  ...
  // rib
  assign s_rib_wr_hdshk  = rib.valid && (~s_rib_ready_q) && rib.wstrb[0];
  assign s_rib_rd_hdshk  = rib.valid && (~s_rib_ready_q) && (~(|rib.wstrb));
  assign rib.ready       = s_rib_ready_q;
  assign rib.rdata       = s_rib_rdata_q;


  assign s_dvp_recven_en = s_rib_wr_hdshk && rib.addr[7:0] == `RIB_DVP_RECVEN;
  assign s_dvp_recven_d  = rib.wdata[0];
  ...
  always_comb begin
    ...
    unique case (rib.addr[7:0])
      `RIB_DVP_RECVEN: s_rib_rdata_d = {31'd0, s_dvp_recven_q};
      `RIB_DVP_RXDATA: begin
        if (s_rib_rd_hdshk) begin
          s_rx_pop_valid = 1'b1;
          if (!s_rx_empty) s_rib_rdata_d = s_rx_pop_data;
          else s_rib_rdata_d = '0;
        end
      end
      `RIB_DVP_STATUS: s_rib_rdata_d = {31'd0, s_dvp_status_q};
