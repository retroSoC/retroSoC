...
module apu_codec_sequencer (
  ...
  assign s_aux            = s_instruction_q[37:32];
  ...
  always_comb begin
    unique case (s_predicate)
      ...
      4'd10:   s_predicate_true = kernel_done_i;
  ...
  always_comb begin
    ...
    s_trap_reason = 8'd0;
    s_trap_detail = 32'd0;
    if (stall_i && (s_watchdog_q + 1'b1 >= timeout_i)) begin
      s_trap_now    = 1'b1;
      s_trap_reason = 8'd7;
    end else if (!instruction_encoding_valid(s_instruction_q)) begin
      s_trap_now    = 1'b1;
      s_trap_reason = 8'd1;
    end else if ((s_class >= 4'd2) || ((s_class == 4'd0) && (s_opcode == 4'd8))) begin
      s_trap_now    = 1'b1;
      s_trap_reason = 8'd6;
  ...
  begin                                                                              \
    ...
    s_stat_q       <= {11'd0, |s_loop_active_q, stall_i, s_opcode, s_class, s_pc_q}; \
  end
