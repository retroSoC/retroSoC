...
module apu_microcode_loader #(
    ...
    parameter int unsigned PathMemoDepth        = 8192,
    parameter int unsigned PathTraversalLimitV1 = 131072,
    parameter int unsigned PathTraversalLimitV2 = 262144,
    ...
    output logic [11:0]           store_addr_o,
    ...
    output logic [2:0][11:0]      entry_pc_o,
    output logic [2:0][11:0]      entry_first_o,
    output logic [2:0][11:0]      entry_last_o,
    ...
    output logic [18:0]           proof_visit_count_o,
    output logic                  proof_memo_full_o,
    output logic                  idle_o
    // verilog_format: on
);
  import apu_microcode_pkg::*;

  localparam logic [12:0] PathStackLimit = 13'(PathStackDepth);
  localparam logic [12:0] PathMemoLast = 13'(PathMemoDepth - 1);

`ifndef SYNTHESIS
  initial begin
    if (EnableP5 && !EnableP4) $fatal(1, "apu_microcode_loader: P5 requires the P4 profile");
    if ((PathMemoDepth == 0) || (PathMemoDepth > 8192) ||
        ((PathMemoDepth & (PathMemoDepth - 1)) != 0)) begin
      $fatal(1, "apu_microcode_loader: path memo depth must be a power of two up to 8192");
    end
  end
`endif
  ...
  logic [12:0]       s_path_stack_count_q;
  logic [18:0]       s_path_visit_q;
  ...
  logic [18:0]       s_path_visit_limit;
  logic s_scan_control_err_q, s_scan_capability_err_q;
  logic [11:0] s_scan_control_pc_q;
  ...
  logic [12:0] s_instruction_word_relative;
  ...
  logic [11:0] s_path_err_pc;
  logic        s_path_fallthrough;
  logic        s_scan_capability_instruction;
  logic [12:0] s_scan_target;

  assign s_header_v2 = EnableP5 && (s_header_q[1] == 32'h0002_0000);

  function automatic logic [2:0] saved_call_depth(input logic [3:0][11:0] return_pc_i);
    logic [2:0] s_depth;
    begin
      s_depth = 3'd0;
      for (int index = 0; index < 4; index++) begin
        if (return_pc_i[index] != 12'd0) s_depth = 3'(index + 1);
      end
      return s_depth;
    end
  endfunction

  function automatic logic [12:0] path_memo_hash(input logic [63:0] key_i);
    logic [63:0] s_mixed;
    begin
      s_mixed = key_i * 64'h9e37_79b9_7f4a_7c15;
      if (PathMemoDepth == 1) return 13'd0;
      return s_mixed[63:51] & PathMemoLast;
    end
  endfunction
  ...
  assign store_addr_o = (s_state_q inside {
    ScanRequest, ScanWait, ScanCheck, PathRequest, PathWait, PathCheck
      ...
      s_scan_pc_q : s_instruction_word_relative[12:1];
  ...
  for (genvar entry = 0; entry < 3; entry++) begin : gen_entry_outputs
    assign entry_pc_o[entry]             = s_header_v2 ?
      s_entry_word_q[entry*8][15:4] : {1'b0, s_entry_word_q[entry*8][14:4]};
    assign entry_first_o[entry]          = s_header_v2 ?
      s_entry_word_q[(entry*8)+1][11:0] : {1'b0, s_entry_word_q[(entry*8)+1][10:0]};
    assign entry_last_o[entry]           = s_header_v2 ?
      s_entry_word_q[(entry*8)+1][27:16] : {1'b0, s_entry_word_q[(entry*8)+1][26:16]};
  ...
  assign s_instruction_word_relative = 13'((s_byte_offset_q - s_header_q[4]) >> 2);
  ...
  always_comb begin
    ...
    if (s_header_q[0] != `APB4_APU__APUMC_MAGIC) begin
      s_header_err = 1'b1;
    end else if (!((s_header_q[1] == `APB4_APU__APUMC_ABI) ||
                   (EnableP5 && (s_header_q[1] == 32'h0002_0000)))) begin
      s_header_err    = 1'b1;
  ...
  always_comb begin
    ...
    if ((s_header_q[3] == 32'd0) || (s_header_q[3] > (s_header_v2 ? 32'd4096 : 32'd2048))) begin
  ...
  always_comb begin
    ...
    for (int unsigned entry = 0; entry < 3; entry++) begin
      ...
      if (!s_range_err && ((s_header_v2 ?
                            (s_entry_word_q[entry*8][31:16] != 16'd0) :
                            (s_entry_word_q[entry*8][31:15] != 17'd0)) ||
        ...
        s_range_detail = s_entry_word_q[entry*8];
                   ...
                   ((s_header_v2 ?
                     ((s_entry_word_q[(entry*8)+1][31:28] != 4'd0) ||
                      (s_entry_word_q[(entry*8)+1][15:12] != 4'd0)) :
                     ((s_entry_word_q[(entry*8)+1][31:27] != 5'd0) ||
                      (s_entry_word_q[(entry*8)+1][15:11] != 5'd0))) ||
                    (entry_first_o[entry] > entry_pc_o[entry]) ||
                    (entry_pc_o[entry] > entry_last_o[entry]) ||
                    ({20'd0, entry_last_o[entry]} >= s_header_q[3]))) begin
        s_range_err    = 1'b1;
  ...
  assign s_scan_immediate = s_header_v2 ?
      s_scan_instruction[11:0] : {1'b0, s_scan_instruction[10:0]};
  assign s_scan_last = s_scan_pc_q == entry_last_o[s_scan_entry_q];
  assign s_scan_target = {1'b0, s_scan_pc_q} + 13'd1 + {1'b0, s_scan_immediate};
  ...
  assign s_path_stack_cs = s_path_stack_push ||
      ((s_state_q == PathPop) && (s_path_stack_count_q != 13'd0));
  assign s_path_stack_write = s_path_stack_push;
  assign s_path_stack_addr = s_path_stack_push ?
      s_path_stack_count_q[11:0] : s_path_stack_count_q[11:0] - 1'b1;
  assign s_path_stack_write_data = {s_loop_active_q, s_return_pc_q, s_scan_pc_q + 1'b1};
  assign s_path_memo_key = {s_loop_active_q, s_return_pc_q, s_scan_pc_q};
...
`ifdef HAVE_SRAM_MACRO
  logic [1:0] s_path_stack_read_bank_q;
  logic [3:0][31:0] s_path_stack_low_data, s_path_stack_high_data;

  tc_sram_1024x32 u_path_stack_low_bank0 (
      .clk_i (clk_i),
      .cs_i  (s_path_stack_cs && (s_path_stack_addr[11:10] == 2'd0)),
  ...
  tc_sram_1024x32 u_path_stack_low_bank1 (
      .clk_i (clk_i),
      .cs_i  (s_path_stack_cs && (s_path_stack_addr[11:10] == 2'd1)),
      .addr_i(s_path_stack_addr[9:0]),
      .data_i(s_path_stack_write_data[31:0]),
      .mask_i(4'hf),
      .wren_i(s_path_stack_write),
  ...
  if (PathStackDepth == 4096) begin : gen_path_stack_expanded
    tc_sram_1024x32 u_path_stack_low_bank2 (
        .clk_i (clk_i),
        .cs_i  (s_path_stack_cs && (s_path_stack_addr[11:10] == 2'd2)),
        .addr_i(s_path_stack_addr[9:0]),
        .data_i(s_path_stack_write_data[31:0]),
        .mask_i(4'hf),
        .wren_i(s_path_stack_write),
        .data_o(s_path_stack_low_data[2])
    );
    tc_sram_1024x32 u_path_stack_high_bank2 (
        .clk_i (clk_i),
        .cs_i  (s_path_stack_cs && (s_path_stack_addr[11:10] == 2'd2)),
        .addr_i(s_path_stack_addr[9:0]),
        .data_i(s_path_stack_write_data[63:32]),
        .mask_i(4'hf),
        .wren_i(s_path_stack_write),
        .data_o(s_path_stack_high_data[2])
    );
    tc_sram_1024x32 u_path_stack_low_bank3 (
        .clk_i (clk_i),
        .cs_i  (s_path_stack_cs && (s_path_stack_addr[11:10] == 2'd3)),
        .addr_i(s_path_stack_addr[9:0]),
        .data_i(s_path_stack_write_data[31:0]),
        .mask_i(4'hf),
        .wren_i(s_path_stack_write),
        .data_o(s_path_stack_low_data[3])
    );
    tc_sram_1024x32 u_path_stack_high_bank3 (
        .clk_i (clk_i),
        .cs_i  (s_path_stack_cs && (s_path_stack_addr[11:10] == 2'd3)),
        .addr_i(s_path_stack_addr[9:0]),
        .data_i(s_path_stack_write_data[63:32]),
        .mask_i(4'hf),
        .wren_i(s_path_stack_write),
        .data_o(s_path_stack_high_data[3])
    );
  end
  ...
  always_ff @(posedge clk_i or negedge rst_n_i) begin
    if (!rst_n_i) begin
      s_path_stack_read_bank_q <= 2'd0;
    end else if (s_path_stack_cs && !s_path_stack_write) begin
      s_path_stack_read_bank_q <= s_path_stack_addr[11:10];
    end
  end
`else
...
`endif
  ...
  assign s_scan_instruction_err = !instruction_encoding_valid(
      s_scan_instruction, s_header_v2
  ) || (s_scan_class >= 4'd7);
  assign s_scan_target_err = (s_scan_class == 4'd0) &&
      (s_scan_opcode inside {4'd3, 4'd4}) &&
      (s_scan_target[12] || (s_scan_target[11:0] < entry_first_o[s_scan_entry_q]) ||
       (s_scan_target[11:0] > entry_last_o[s_scan_entry_q]));
      ...
      (((s_scan_opcode == 4'd6) && s_loop_active_q[s_scan_aux[1:0]]) ||
       ((s_scan_opcode == 4'd7) &&
        (!s_loop_active_q[s_scan_aux[1:0]] ||
         ...
         ({1'b0, s_scan_pc_q} + 13'd1 < {1'b0, s_scan_immediate}) ||
        ...
        unique case (s_state_q)
          ...
          ScanCheck: begin
            if (s_scan_instruction_err || s_scan_target_err || s_scan_loop_err ||
              ...
              s_scan_control_pc_q <= s_scan_pc_q;
              s_scan_control_addr_q <= s_source_addr_q + s_header_q[4] + {17'd0, s_scan_pc_q, 3'd0};
              ...
              s_state_q <= EnableP5 ? PathMemoClear : PathRequest;
            end else begin
              if (s_scan_pc_q == entry_pc_o[s_scan_entry_q]) begin
                s_entry_instruction_q[s_scan_entry_q] <= s_scan_instruction;
              end
              if (!s_scan_capability_err_q && s_scan_capability_instruction &&
                ...
                s_scan_capability_addr_q <= s_source_addr_q + s_header_q[4] +
                    {17'd0, s_scan_pc_q, 3'd0};
                s_scan_capability_detail_q <= trap_detail(
                    8'd6, s_scan_pc_q[10:0], s_scan_instruction
                );
              end
          ...
          PathMemoStart: begin
            if (s_call_depth_q > 3'd2) begin
              ...
              s_path_memo_addr_q  <= path_memo_hash(s_path_memo_key);
              s_path_memo_probe_q <= 13'd0;
          ...
          PathMemoCheck: begin
            if (s_path_memo_read_valid_q && (s_path_memo_read_key_q == s_path_memo_key)) begin
              ...
              s_path_memo_addr_q  <= s_path_memo_addr_q + 1'b1;
              s_path_memo_probe_q <= s_path_memo_probe_q + 1'b1;
              s_state_q           <= PathMemoRead;
            end
          end
          PathRequest: begin
            if (s_path_visit_q >= s_path_visit_limit) begin
              if (!s_scan_control_err_q || (entry_pc_o[s_scan_entry_q] < s_scan_control_pc_q)) begin
                s_scan_control_err_q <= 1'b1;
                s_scan_control_pc_q <= entry_pc_o[s_scan_entry_q];
                s_scan_control_addr_q <= s_source_addr_q + s_header_q[4] +
                    {17'd0, entry_pc_o[s_scan_entry_q], 3'd0};
                s_scan_control_detail_q <= trap_detail(
                    8'd2, entry_pc_o[s_scan_entry_q], s_entry_instruction_q[s_scan_entry_q]
                );
              end
              s_path_stack_count_q <= 13'd0;
              s_state_q            <= PathPop;
            end else begin
              s_path_visit_q <= s_path_visit_q + 1'b1;
              s_state_q      <= PathWait;
            end
          end
          PathWait: begin
          ...
          PathCheck: begin
            if (s_path_stop_valid_q && (s_scan_pc_q == s_path_stop_pc_q)) begin
              ...
              if (!s_scan_control_err_q || (s_path_err_pc < s_scan_control_pc_q)) begin
                s_scan_control_err_q <= 1'b1;
                s_scan_control_pc_q <= s_path_err_pc;
                s_scan_control_addr_q <= s_source_addr_q + s_header_q[4] +
                    {17'd0, s_path_err_pc, 3'd0};
                s_scan_control_detail_q <= trap_detail(
                    s_path_err_reason, s_path_err_pc[10:0], s_scan_instruction
                );
              end
              s_state_q <= PathPop;
              ...
              s_scan_pc_q <= s_scan_pc_q + 1'b1;
              s_state_q   <= EnableP5 ? PathMemoStart : PathRequest;
              ...
              if ((s_scan_predicate != 4'd0) && (s_path_stack_count_q == PathStackLimit)) begin
                ...
                s_scan_control_addr_q <= s_source_addr_q + s_header_q[4] +
                    {17'd0, entry_pc_o[s_scan_entry_q], 3'd0};
                s_scan_control_detail_q <= trap_detail(
                    8'd2, entry_pc_o[s_scan_entry_q][10:0], s_scan_instruction
                );
                s_state_q <= PathPop;
              end else begin
          ...
          PathPop: begin
            if (s_path_stack_count_q != 13'd0) begin
              s_path_stack_count_q <= s_path_stack_count_q - 1'b1;
          ...
          PathPopLoad: begin
            s_scan_pc_q     <= s_path_stack_read_data[11:0];
            s_call_depth_q  <= saved_call_depth(s_path_stack_read_data[59:12]);
            s_return_pc_q   <= s_path_stack_read_data[59:12];
            s_loop_active_q <= s_path_stack_read_data[63:60];
            s_state_q       <= EnableP5 ? PathMemoStart : PathRequest;
          end
          Finish: begin
            s_state_q <= Idle;
