...
module apu_local_sram #(
  ...
  localparam int unsigned LogicalBankCount = 28;
  localparam int unsigned KwsBaseWord = `APB4_APU__LOCAL_KWS_BASE / 4;
  localparam int unsigned KwsWordCount = `APB4_APU__LOCAL_KWS_BYTES / 4;
  localparam int unsigned InternalBaseWord = `APB4_APU__LOCAL_INTERNAL_BASE / 4;
  localparam int unsigned PhysicalWordCount =
      (MemoryWordCount == (`APB4_APU__LOCAL_DATA_BYTES / 4)) ?
      MemoryWordCount - KwsWordCount : MemoryWordCount;
  ...
  logic [                 13:0] s_physical_word_addr;
  logic                         s_physical_word_selected;
  logic                         s_codec_range_ok;
  logic                         s_codec_table;
  logic                         s_codec_mutable_valid;
  logic                         s_codec_access_err;
  logic [PhysicalWordCount-1:0] s_mutable_valid_q;
  ...
  assign s_physical_word_selected =
      (MemoryWordCount != (`APB4_APU__LOCAL_DATA_BYTES / 4)) ?
      (s_word_addr < MemoryWordCountValue) :
      ((s_word_addr < 15'(KwsBaseWord)) ||
       ((s_word_addr >= 15'(InternalBaseWord)) && (s_word_addr < MemoryWordCountValue)));
  assign s_physical_word_addr = (s_word_addr < 15'(KwsBaseWord)) ?
      14'(s_word_addr) : ((s_word_addr >= 15'(InternalBaseWord)) ?
                          14'(s_word_addr - 15'(KwsWordCount)) : 14'd0);
  ...
  assign s_codec_mutable_valid = s_physical_word_selected &&
      s_mutable_valid_q[s_physical_word_addr];
...
`ifdef HAVE_SRAM_MACRO
  localparam int unsigned PhysicalBankCount = LogicalBankCount - 16;

  logic [                  4:0]       s_bank;
  logic [                  3:0]       s_physical_bank;
  logic [                  9:0]       s_bank_addr;
  logic [PhysicalBankCount-1:0][31:0] s_bank_data;
  logic [                  3:0]       s_read_bank_q;

  assign s_bank          = s_word_addr[14:10];
  assign s_physical_bank = (s_bank < 5'd10) ? 4'(s_bank) : 4'(s_bank - 5'd16);
  assign s_bank_addr     = s_word_addr[9:0];

  for (genvar bank = 0; bank < PhysicalBankCount; bank++) begin : gen_local_bank
    localparam logic [3:0] BankIndex = bank;
    logic s_bank_selected;

    assign s_bank_selected = s_req && s_physical_word_selected &&
        (s_physical_bank == BankIndex) && (loader_active_i || !s_codec_access_err);

    tc_sram_1024x32 u_local_sram (
        .clk_i (clk_i),
        .cs_i  (s_bank_selected),
        .addr_i(s_bank_addr),
        .data_i(s_write_data),
        .mask_i(s_write_strb),
        .wren_i(s_write),
  ...
  always_ff @(posedge clk_i or negedge rst_n_i) begin
    if (!rst_n_i) begin
      s_read_bank_q <= 4'd0;
    end else if (s_req && !s_write && s_physical_word_selected &&
                 (loader_active_i || !s_codec_access_err)) begin
      s_read_bank_q <= s_physical_bank;
    end
  end
`else
  logic [31:0] mem           [0:PhysicalWordCount-1];
  logic [31:0] s_read_data_q;

  assign s_read_data = s_read_data_q;

  always_ff @(posedge clk_i) begin
    if (s_req && s_physical_word_selected && (loader_active_i || !s_codec_access_err)) begin
      if (s_write) begin
        if (s_write_strb[0]) mem[s_physical_word_addr][7:0] <= s_write_data[7:0];
        if (s_write_strb[1]) mem[s_physical_word_addr][15:8] <= s_write_data[15:8];
        if (s_write_strb[2]) mem[s_physical_word_addr][23:16] <= s_write_data[23:16];
        if (s_write_strb[3]) mem[s_physical_word_addr][31:24] <= s_write_data[31:24];
      end else begin
        s_read_data_q <= mem[s_physical_word_addr];
      end
    end
  end
`endif
  ...
  always_ff @(posedge clk_i or negedge rst_n_i) begin
    if (!rst_n_i) begin
      s_mutable_valid_q <= PhysicalWordCount'(0);
      s_read_q          <= 1'b0;
      s_read_codec_q    <= 1'b0;
      s_read_err_q      <= 1'b0;
    end else begin
      ...
      if (epoch_clear_i) begin
        s_mutable_valid_q <= PhysicalWordCount'(0);
                   ...
                   !s_codec_access_err && s_physical_word_selected) begin
        s_mutable_valid_q[s_physical_word_addr] <= 1'b1;
      end
    end
  end
