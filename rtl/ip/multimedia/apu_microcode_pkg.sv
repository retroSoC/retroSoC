...
package apu_microcode_pkg;
  ...
  function automatic logic [31:0] trap_detail(input logic [7:0] reason_i, input logic [11:0] pc_i,
                                              input logic [63:0] instruction_i);
    logic s_unused;
    begin
      s_unused = ^{instruction_i[55:37], instruction_i[31:0]};
      return {instruction_i[36:32], instruction_i[59:56], instruction_i[63:60],
              pc_i[10:0], reason_i} |
          {32{s_unused && 1'b0}};
    end
  endfunction
    ...
    begin
      ...
      if (s_class == `APB4_APU__MC_CLASS_CONTROL) begin
        unique case (s_opcode)
          ...
          s_valid = s_valid && ({s_dst, s_src0, s_src1, s_aux} == 20'd0) &&
              (v2_i ? (s_immediate[31:12] == 20'd0) :
                     (s_immediate[31:11] == 21'd0)) &&
              (v2_i ? (s_immediate[11:0] != 12'd0) : (s_immediate[10:0] != 11'd0));
