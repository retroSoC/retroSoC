...
module clint_core #(
    ...
    input  logic [63:0]         mtimecmp_i [0:HART_NUM-1],
    output logic [63:0]         mtime_o,
    output logic [HART_NUM-1:0] timer_irq_o
    // verilog_format: on
);

  logic [HART_NUM-1:0] s_timer_irq_d, s_timer_irq_q;

  initial begin
    if ((HART_NUM < 1) || (HART_NUM > 4095)) begin
      $fatal(1, "clint_core: HART_NUM must be between 1 and 4095");
    end
  end
