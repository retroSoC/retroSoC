...
`ifndef RIBP_TIMER_DEF_SV
`define RIBP_TIMER_DEF_SV

// verilog_format: off
`define RIBP_TIMER_CFG 8'h00
`define RIBP_TIMER_RLD 8'h04
`define RIBP_TIMER_VAL 8'h08
// verilog_format: on
...
module ribp_timer (
  ...
  // register
  assign s_tim_cfg_en    = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_TIMER_CFG;
  assign s_tim_cfg_d     = ribp.wdata[3:0];
  dffer #(4) u_tim_cfg_dffer (
      clk_i,
      rst_n_i,
      s_tim_cfg_en && ribp.wstrb[0],
      s_tim_cfg_d,
      s_tim_cfg_q
  );


  assign s_tim_rld_en = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_TIMER_RLD;
  always_comb begin
    s_tim_rld_d = s_tim_rld_q;
    if (ribp.wstrb[0]) s_tim_rld_d[7:0] = ribp.wdata[7:0];
    if (ribp.wstrb[1]) s_tim_rld_d[15:8] = ribp.wdata[15:8];
    if (ribp.wstrb[2]) s_tim_rld_d[23:16] = ribp.wdata[23:16];
    if (ribp.wstrb[3]) s_tim_rld_d[31:24] = ribp.wdata[31:24];
  end
  ...
  always_comb begin
    ...
    s_irq_d     = s_irq_q;
    if (s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_TIMER_VAL) begin
      if (ribp.wstrb[0]) s_tim_val_d[7:0] = ribp.wdata[7:0];
      if (ribp.wstrb[1]) s_tim_val_d[15:8] = ribp.wdata[15:8];
      if (ribp.wstrb[2]) s_tim_val_d[23:16] = ribp.wdata[23:16];
      if (ribp.wstrb[3]) s_tim_val_d[31:24] = ribp.wdata[31:24];
  ...
  always_comb begin
    ...
    unique case (ribp.addr[7:0])
      `RIBP_TIMER_CFG: s_ribp_rdata_d = {28'd0, s_tim_cfg_q};
      `RIBP_TIMER_RLD: s_ribp_rdata_d = s_tim_rld_q;
      `RIBP_TIMER_VAL: s_ribp_rdata_d = s_tim_val_q;
