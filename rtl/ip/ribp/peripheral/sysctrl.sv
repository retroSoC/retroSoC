...
module ribp_sysctrl (
  ...
  assign s_user_reset_en = s_ribp_wr_hdshk && ribp.addr[7:0] == SYSCTRL_USER_CORE_RESET_OFFSET &&
                           ribp.wstrb[0];
  assign s_user_config_error_clear = s_ribp_wr_hdshk &&
  ...
  assign s_pll_clear_error = s_ribp_wr_hdshk && ribp.addr[7:0] == SYSCTRL_PLL_CMD_OFFSET &&
  ...
  assign s_perf_enable_en = s_ribp_wr_hdshk && ribp.addr[7:0] == SYSCTRL_PERF_CTRL_OFFSET &&
                            ribp.wstrb[0];
  assign s_perf_enable_d = ribp.wdata[0];
  assign s_perf_clear = s_perf_enable_en && ribp.wdata[1];
  assign s_perf_snapshot = s_perf_enable_en && ribp.wdata[2];
  ...
  dffer #(64) u_perf_ribp_wait_dffer (
      clk_i,
      rst_n_i,
      s_perf_snapshot,
      sysctrl.perf_ribp_wait_i,
      s_perf_ribp_wait_q
  );
  ...
  always_comb begin
    ...
    unique case (ribp.addr[7:0])
      ...
      SYSCTRL_PERF_CTRL_OFFSET:          s_ribp_rdata_d = {31'd0, s_perf_enable_q};
