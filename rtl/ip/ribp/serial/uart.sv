...
`ifndef RIBP_UART_DEF_SV
`define RIBP_UART_DEF_SV

// verilog_format: off
`define RIBP_UART_DIV 8'h00
`define RIBP_UART_DAT 8'h04
// verilog_format: on
...
module ribp_uart (
  ...
  assign s_uart_div_en   = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_UART_DIV;
  always_comb begin
    s_uart_div_d = s_uart_div_q;
    if (ribp.wstrb[0]) s_uart_div_d[7:0] = ribp.wdata[7:0];
    if (ribp.wstrb[1]) s_uart_div_d[15:8] = ribp.wdata[15:8];
    if (ribp.wstrb[2]) s_uart_div_d[23:16] = ribp.wdata[23:16];
    if (ribp.wstrb[3]) s_uart_div_d[31:24] = ribp.wdata[31:24];
  end
  ...
  assign s_uart_dat_en   = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_UART_DAT;
  assign s_send_dat_wait = s_uart_dat_en && ribp.wstrb[0] && ((|r_send_bitcnt) || r_send_dummy);
  assign s_ribp_ready_d  = ribp.valid && (~s_ribp_ready_q) && (~s_send_dat_wait);
  ...
  always_comb begin
    ...
    unique case (ribp.addr[7:0])
      `RIBP_UART_DIV: s_ribp_rdata_d = s_uart_div_q;
      `RIBP_UART_DAT: s_ribp_rdata_d = r_recv_buf_valid ? {24'd0, r_recv_buf_data} : '1;
  ...
  always_ff @(posedge clk_i, negedge rst_n_i) begin
    if (!rst_n_i) begin
      ...
      if (r_send_dummy && (~(|r_send_bitcnt))) begin
        ...
        r_send_divcnt  <= '0;
        r_send_dummy   <= '0;
      end else if (s_uart_dat_en && ribp.wstrb[0] && (~(|r_send_bitcnt))) begin
        r_send_pattern <= {1'b1, ribp.wdata[7:0], 1'b0};
`ifndef SYNTHESIS
`ifdef SIMU_VERILATOR
        $write("%c", ribp.wdata[7:0]);  // NOTE: just for verilator sim
`endif
`endif
        r_send_bitcnt <= 4'd10;
        r_send_divcnt <= '0;
