...
// verilog_format: off
`define RIBP_ONEWIRE_CLKDIV  8'h00
`define RIBP_ONEWIRE_ZEROCNT 8'h04
`define RIBP_ONEWIRE_ONECNT  8'h08
`define RIBP_ONEWIRE_RSTCNT  8'h0C
`define RIBP_ONEWIRE_TXDATA  8'h10
`define RIBP_ONEWIRE_CTRL    8'h14
`define RIBP_ONEWIRE_STATUS  8'h18
// verilog_format: on
...
module ribp_onewire (
  ...
  assign s_onewire_clkdiv_en = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_CLKDIV;
  assign s_onewire_clkdiv_d  = ribp.wdata[7:0];
  ...
  assign s_onewire_zerocnt_en = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_ZEROCNT;
  assign s_onewire_zerocnt_d  = ribp.wdata[7:0];
  ...
  assign s_onewire_onecnt_en = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_ONECNT;
  assign s_onewire_onecnt_d  = ribp.wdata[7:0];
  ...
  assign s_onewire_rstcnt_en = s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_RSTCNT;
  assign s_onewire_rstcnt_d  = ribp.wdata[7:0];
  ...
  always_comb begin
    ...
    s_tx_push_data  = '0;
    if (s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_TXDATA) begin
      s_tx_push_valid = 1'b1;
      if (ribp.wstrb[0]) s_tx_push_data[7:0] = ribp.wdata[7:0];
      if (ribp.wstrb[1]) s_tx_push_data[15:8] = ribp.wdata[15:8];
      if (ribp.wstrb[2]) s_tx_push_data[23:16] = ribp.wdata[23:16];
    end
  end
  ...
  always_comb begin
    if (s_ribp_wr_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_CTRL) begin
      s_onewire_ctrl_d = ribp.wdata[1:0];
    end else begin
      s_onewire_ctrl_d = '0;
    end
  end
  ...
  always_comb begin
    ...
    end else if (s_ribp_rd_hdshk && ribp.addr[7:0] == `RIBP_ONEWIRE_STATUS) begin
  ...
  always_comb begin
    ...
    unique case (ribp.addr[7:0])
      `RIBP_ONEWIRE_CLKDIV:  s_ribp_rdata_d = {24'd0, s_onewire_clkdiv_q};
      `RIBP_ONEWIRE_ZEROCNT: s_ribp_rdata_d = {24'd0, s_onewire_zerocnt_q};
      `RIBP_ONEWIRE_ONECNT:  s_ribp_rdata_d = {24'd0, s_onewire_onecnt_q};
      `RIBP_ONEWIRE_RSTCNT:  s_ribp_rdata_d = {24'd0, s_onewire_rstcnt_q};
      `RIBP_ONEWIRE_STATUS:  s_ribp_rdata_d = {29'd0, s_onewire_status_q};
