...
# testbench filelist
TB_FLIST     := -f $(RTL_PATH)/.generated_fl/tb.fl
...
COMMON_OPTS  := -full64 +v2k -sverilog -timescale=1ns/10ps \
                ...
                -kdb \
                -debug_access+all \
                -msg_config=$(RTL_PATH)/.generated_fl/lint.msg \
                +error+500 \
                +vcs+flush+all \
...
netsim:   DIR   := .vcs_build/netl
postsim:  DIR   := .vcs_build/post

wave:     DIR   := .vcs_build/behv
netwave:  DIR   := .vcs_build/netl
postwave: DIR   := .vcs_build/post
