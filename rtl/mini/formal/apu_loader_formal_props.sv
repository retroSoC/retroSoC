  ...
  always @(posedge clk_i) begin
    if (rst_n_i) begin
      ...
      if (store_write) begin
        assert (status[0]);
        assert (request_count == 2'd3);
        assert (store_addr[11:0] == store_addr);
      end
      ...
      if (scenario >= 4'd7) begin
        assert (proof_visits <= ((scenario == 4'd8) ? 19'd1 : 19'd16));
      end
      if ((scenario == 4'd0) || (scenario == 4'd7) || (scenario == 4'd11)) begin
        assert (!fault_valid || (fault_code != 6'd9));
      end else if (scenario == 4'd1) begin
      ...
      end else if (scenario == 4'd2) begin
        if (fault_valid) begin
      ...
      end else if (scenario == 4'd8) begin
        if (fault_valid) begin
          assert (status[4] && !lock && (fault_code == 6'd9));
          assert ((fault_stage == 4'd1) && (fault_resp == 2'd0) && (fault_addr == 32'h0000_10c0));
          assert (fault_detail[7:0] == 8'd2);
        end
      end else if ((scenario == 4'd6) || (scenario == 4'd10)) begin
        if (fault_valid) begin
    ...
    cover (rst_n_i &&
           (((scenario == 4'd0) && load_seen && store_seen && status[1] && lock) ||
            ((scenario == 4'd1) && fault_valid && status[3]) ||
            ((scenario == 4'd2) && fault_valid && status[3]) ||
            ((scenario == 4'd3) && fault_valid && status[6]) ||
            ((scenario == 4'd4) && fault_valid && status[4]) ||
            ((scenario == 4'd5) && abort_seen) ||
            ((scenario == 4'd6) && fault_seen) ||
            ((scenario == 4'd7) && load_seen && store_seen && status[1] && lock) ||
            ((scenario == 4'd8) && fault_valid && status[4]) ||
            ((scenario == 4'd9) && abort_seen) ||
            ((scenario == 4'd10) && fault_seen) ||
            ((scenario == 4'd11) && memo_full_seen && (proof_visits >= 19'd2))));
  end
endmodule
