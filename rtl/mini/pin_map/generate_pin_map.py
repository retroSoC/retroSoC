...
def render_pad_instance(pad: Pad) -> str | None:
    ...
    if pad.kind == "tri_input":
        return (
            f"  tc_io_tri_pad {instance} (.pad({pad.name}), .c2p(1'b0), .c2p_en(1'b0), "
            f".p2c({pad.signal}));"
        )
    if pad.kind == "schmitt_input":
        return (
            f"  tc_io_tri_schmitt_pad {instance} (.pad({pad.name}), .c2p(1'b0), "
            f".c2p_en(1'b0), .p2c({pad.signal}));"
        )
    ...
    if pad.kind == "full":
        return (
            f"  tc_io_tri_full_pad {instance} (.pad({pad.name}), .c2p({pad.interface}.do_o[{pad.index}]), "
    ...
    if pad.kind == "xpi_data":
        return (
            f"  tc_io_tri_pad {instance} (.pad({pad.name}), .c2p(u_xpi_if.io_do_o[{pad.index}]), "
            f".c2p_en(u_xpi_if.io_oe_o[{pad.index}]), .p2c(u_xpi_if.io_di_i[{pad.index}]));"
        )
    if pad.kind == "sdram_dq":
        return (
            f"  tc_io_tri_pad {instance} (.pad({pad.name}), .c2p(u_sdram_if.dq_o[{pad.index}]), "
            f".c2p_en(u_sdram_if.oe_o), .p2c(u_sdram_if.dq_i[{pad.index}]));"
        )
    if pad.kind == "xtal" and pad.bind:
        return (
            f"  tc_io_xtl_pad u_xtal_io_pad (.xi_pad({pad.name}), .xo_pad({pad.peer}), "
            f".en(1'b1), .clk({pad.signal}));"
        )
    return None
