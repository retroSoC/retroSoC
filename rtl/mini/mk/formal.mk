...
FORMAL_APU_P5_LOADER_COVER_DEPTH ?= 160
FORMAL_APU_P5_LOADER_COVER_SKIP  = $(if $(filter apu_loader_p5_success,$*),130,$(if $(filter apu_loader_p5_bound,$*),110,$(if $(filter apu_loader_p5_memo_fallback,$*),115,30)))
FORMAL_APU_SEQUENCER_DEPTH       ?= 38
FORMAL_APU_KWS_DEPTH             ?= 32
FORMAL_APU_P9_DEPTH              ?= 40
...
FORMAL_APU_P5_LOADER_TIMEOUT     ?= 600
...
FORMAL_TARGET_TIMEOUT            = $(if $(filter ga2d,$*),$(FORMAL_GA2D_TIMEOUT),$(if $(filter npu_dma,$*),$(FORMAL_NPU_DMA_TIMEOUT),$(if $(filter npu_context npu_control,$*),$(FORMAL_NPU_P6_TIMEOUT),$(if $(filter $(FORMAL_APU_P5_LOADER_TARGETS),$*),$(FORMAL_APU_P5_LOADER_TIMEOUT),$(if $(filter dma,$*),$(FORMAL_DMA_TIMEOUT),$(if $(filter apu apu_codec apu_kws apu_p9 apu_primitives apu_sequencer $(FORMAL_APU_LOADER_TARGETS) $(FORMAL_APU_PRIMITIVE_TARGETS),$*),$(FORMAL_APU_TIMEOUT),$(if $(filter gateway_a,$*),$(FORMAL_GATEWAY_A_TIMEOUT),$(if $(filter sdio,$*),$(FORMAL_SDIO_TIMEOUT),$(if $(filter opipsram,$*),$(FORMAL_OPIPSRAM_TIMEOUT),$(if $(filter ws2812,$*),$(FORMAL_WS2812_TIMEOUT),$(if $(filter i2c,$*),$(FORMAL_I2C_TIMEOUT),$(FORMAL_TIMEOUT))))))))))))
FORMAL_APU_TARGET_DEPTH          = $(if $(filter $(FORMAL_APU_P5_LOADER_TARGETS),$*),$(FORMAL_APU_P5_LOADER_DEPTH),$(if $(filter $(FORMAL_APU_LOADER_TARGETS),$*),$(FORMAL_APU_LOADER_DEPTH),$(if $(filter apu_p9,$*),$(FORMAL_APU_P9_DEPTH),$(if $(filter apu_kws,$*),$(FORMAL_APU_KWS_DEPTH),$(if $(filter apu_sequencer,$*),$(FORMAL_APU_SEQUENCER_DEPTH),$(if $(filter apu_codec,$*),$(FORMAL_APU_CODEC_DEPTH),$(FORMAL_APU_DEPTH)))))))
...
FORMAL_TARGETS                   := bus rib_adapter rib2apb sysctrl pll_rcu gpio ws2812 uart i2c timer clint dvp i2s psram onchip_ram opipsram dma ga2d apu apu_codec apu_kws apu_p9 $(FORMAL_APU_PRIMITIVE_TARGETS) $(FORMAL_APU_LOADER_TARGETS) apu_sequencer gateway_a sdio npu_dma
...
FORMAL_SOURCE_FILES              := $(RTL_PATH)/formal/bus_formal.sv \
                             ...
                             $(RTL_PATH)/formal/ga2d_formal.sv \
                             $(RTL_PATH)/formal/ga2d_formal_props.sv \
                             $(RTL_PATH)/formal/apu_formal.sv \
                             $(RTL_PATH)/formal/apu_formal_props.sv \
                             $(RTL_PATH)/formal/apu_codec_formal.sv \
                             $(RTL_PATH)/formal/apu_codec_formal_props.sv \
                             $(RTL_PATH)/formal/apu_kws_formal.sv \
                             $(RTL_PATH)/formal/apu_kws_formal_props.sv \
                             $(RTL_PATH)/formal/apu_p9_formal.sv \
                             $(RTL_PATH)/formal/apu_p9_formal_props.sv \
                             $(RTL_PATH)/formal/apu_primitives_formal.sv \
                             $(RTL_PATH)/formal/apu_primitives_formal_props.sv \
                             ...
                             $(ROOT_PATH)/rtl/ip/multimedia/apu_codec_transport.sv \
                             $(ROOT_PATH)/rtl/ip/multimedia/apu_codec_controller.sv \
...
formal-apu-kws: $(FORMAL_DIR)/apu_kws/.stamp | manifest

formal-apu-p9: $(FORMAL_DIR)/apu_p9/.stamp | manifest
...
formal-ga2d: $(FORMAL_DIR)/ga2d/.stamp | manifest

formal-apu: $(FORMAL_DIR)/apu/.stamp $(FORMAL_DIR)/apu_codec/.stamp $(addsuffix /.stamp,$(addprefix $(FORMAL_DIR)/,$(FORMAL_APU_PRIMITIVE_TARGETS))) | manifest

formal-apu-loader: $(addsuffix /.stamp,$(addprefix $(FORMAL_DIR)/,$(FORMAL_APU_LOADER_TARGETS))) | manifest

formal-apu-loader-p5: $(addsuffix /.stamp,$(addprefix $(FORMAL_DIR)/,$(FORMAL_APU_P5_LOADER_TARGETS))) | manifest
...
.PHONY: formal formal-bus formal-rib-adapter formal-rib2apb formal-sysctrl formal-pll-rcu formal-gpio formal-ws2812 formal-uart formal-i2c formal-timer formal-clint formal-dvp formal-i2s formal-psram formal-onchip-ram formal-opipsram formal-dma formal-ga2d formal-apu formal-apu-kws formal-apu-p9 formal-apu-loader formal-apu-loader-p5 formal-apu-sequencer formal-gateway-a formal-sdio formal-npu_dma formal-npu-context formal-npu-control formal-npu-p6 formal-doctor formal-clean
