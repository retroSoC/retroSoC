...
// Management-core AHB-Lite to SoC NMI bridge. It is intentionally separate
// from the managed-compatible ahbl2nmi used by externally supplied user cores.
module ahbl2soc_nmi (
    ahbl_if.slave     ahbl,
  ...
  assign ahbl.hresp  = nmi.ready && nmi.resp_err ? AHBL_RESP_ERROR : AHBL_RESP_OKAY;
      ...
      unique case (s_hsize_q)
        AHBL_SIZE_BYTE: begin
          unique case (s_haddr_q[1:0])
            2'd0: begin
              nmi.wstrb = 4'b0001;
              nmi.wdata = {24'd0, ahbl.hwdata[7:0]};
            end
            2'd1: begin
              nmi.wstrb = 4'b0010;
              nmi.wdata = {16'd0, ahbl.hwdata[7:0], 8'd0};
            end
            2'd2: begin
              nmi.wstrb = 4'b0100;
              nmi.wdata = {8'd0, ahbl.hwdata[7:0], 16'd0};
            end
            default: begin
              nmi.wstrb = 4'b1000;
              nmi.wdata = {ahbl.hwdata[7:0], 24'd0};
            end
          endcase
        end
        AHBL_SIZE_HWRD: begin
          if (s_haddr_q[1]) begin
            ...
            nmi.wstrb = 4'b0011;
            nmi.wdata = {16'd0, ahbl.hwdata[15:0]};
          end
        end
        default: begin
          nmi.wstrb = 4'b1111;
