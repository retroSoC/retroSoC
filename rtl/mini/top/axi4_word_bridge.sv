...
module axi4_word_bridge (
    ...
    input  logic [31:0] req_rdata_i,
    input  logic        req_resp_err_i
    // verilog_format: on
);
  ...
  assign req_valid_o = (s_fsm_q == FSM_WR_REQ) || (s_fsm_q == FSM_RD_REQ);
  assign req_addr_o = s_addr_q;
  assign req_wdata_o = s_wdata_q;
  assign req_wstrb_o = (s_fsm_q == FSM_WR_REQ) ? s_wstrb_q : '0;
    ...
    unique case (s_fsm_q)
      ...
      FSM_WR_REQ: begin
        if (req_ready_i) begin
          if (req_resp_err_i) s_resp_d = `AXI4_RESP_SLAVE_ERROR;
