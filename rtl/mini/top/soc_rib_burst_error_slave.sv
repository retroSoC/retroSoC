...
module soc_rib_burst_error_slave (
    input logic                        clk_i,
    input logic                        rst_n_i,
  ...
  assign burst.w_ready   = s_fsm_q == FSM_WDATA;
  assign burst.rsp_valid = s_fsm_q == FSM_RESP;
  assign burst.rdata     = '0;
  assign burst.resp_err  = 1'b1;
  assign burst.resp_code = s_error_code_q;
  assign burst.rsp_beat  = s_beat_q;
  assign burst.rsp_last  = 1'b1;
  ...
  always_comb begin
    ...
    unique case (s_fsm_q)
      FSM_CMD: begin
        if (burst.cmd_valid) begin
          s_beat_d = '0;
          s_fsm_d  = burst.cmd_write ? FSM_WDATA : FSM_RESP;
        end
      end
      FSM_WDATA: begin
        if (burst.w_valid) begin
