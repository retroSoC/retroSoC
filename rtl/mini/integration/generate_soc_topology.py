...
def parse_data_master_policies(value: Any) -> list[DataMasterPolicy]:
    if not isinstance(value, list) or len(value) != len(DATA_MASTER_NAMES):
        raise ValueError(
            f"data_master_policies must define all {len(DATA_MASTER_NAMES)} data masters"
        )
...
def render_data_policy(policies: list[DataMasterPolicy]) -> str:
    def packed_target_mask(access: str) -> str:
        ...
        return f"{len(policies) * len(DATA_TARGET_NAMES)}'b{bits}"
    ...
    return "\n".join(
        [
            ...
            f"`define {'SOC_DATA_POLICY_ALLOW_INSTRUCTION':<44} {len(policies)}'b{instruction_bits}",
            f"`define {'SOC_DATA_POLICY_REQUIRE_NONCACHEABLE':<44} {len(policies)}'b{noncacheable_bits}",
            "",
        ]
    )
...
def render_irq_metadata(
    vector_width: int, external_irq_count: int, interrupts: list[Interrupt]
) -> str:
    """Render the SDK-facing interrupt capacity and source metadata."""
    external = [interrupt for interrupt in interrupts if interrupt.core_bit >= 2]
    lines = [
        "/* Generated by rtl/mini/integration/generate_soc_topology.py; do not edit. */",
        "#ifndef RETROSOC_GENERATED_IRQ_METADATA_H",
        "#define RETROSOC_GENERATED_IRQ_METADATA_H",
        "",
        "#include <stdint.h>",
        "",
        f"#define RS_SOC_IRQ_VECTOR_WIDTH UINT32_C({vector_width})",
        f"#define RS_SOC_EXTERNAL_IRQ_COUNT UINT32_C({external_irq_count})",
        f"#define RS_SOC_ALLOCATED_IRQ_COUNT UINT32_C({len(external)})",
        "",
    ]
    for interrupt in interrupts:
        name = interrupt.name.upper()
        lines.append(f"#define RS_SOC_IRQ_{name} UINT32_C({interrupt.core_bit})")
        if interrupt.core_bit >= 2:
            lines.append(
                f"#define RS_SOC_EXT_IRQ_{name} UINT32_C({interrupt.core_bit - 2})"
            )
    lines.extend(["", "#endif", ""])
    return "\n".join(lines)
...
def generate(
    ...
    include_dir = output_dir / "include" / "retrosoc" / "generated"
    ...
    atomic_write(
        include_dir / "irq_metadata.h",
        render_irq_metadata(irq_vector_width, external_irq_count, interrupts),
    )
...
def main() -> int:
    ...
    parser.add_argument("--external-irq-count", type=int, default=62)
    parser.add_argument("--check", action="store_true", help="validate only; write nothing")
    arguments = parser.parse_args()
    try:
        if not 1 <= arguments.external_irq_count <= IRQ_VECTOR_WIDTH - 2:
            raise ValueError("external IRQ count must fit the vector width")
        if arguments.check:
