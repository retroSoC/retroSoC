...
def parse_targets(value: Any, field: str, selector_width: int) -> tuple[ExtensionTarget, ...]:
    ...
    for index, entry in enumerate(value):
        ...
        if not isinstance(slot, int) or not 0 < slot < (1 << selector_width):
    ...
    if [target.slot for target in ordered] != list(range(1, len(ordered) + 1)):
        raise ValueError(f"{field} slots must be contiguous from one")
    return ordered
...
def read_extensions(path: Path) -> ExtensionMap:
    ...
    return ExtensionMap(
        ...
        ip_selector_width,
        gpio_width,
        parse_targets(document.get("core_targets"), "core_targets", core_selector_width),
        parse_targets(document.get("ip_targets"), "ip_targets", ip_selector_width),
    )
